        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for i, entity in enumerate(entities, 1):
                    print(f"\n  Entity {i}:")
                    print(f"    ID: {entity.get('entityId') or entity.get('entity_id')}")
...
async def main():
    ...
    print(f"  ExampleEntities: {'✅ Found' if success1 and len(example_entities) > 0 else '❌ Not found'}")
    print(f"  OtherEntities: {'✅ Found' if success2 and len(other_entities) > 0 else '❌ Not found'}")
    
    if success2 and len(other_entities) >= 3:
        print(f"\n  ✅ Processor created {len(other_entities)} OtherEntity instances")
        print(f"     Expected: 3 instances")
