...
from tests.utils.mocks import (MockServices, create_mock_cloud_event,
                               create_mock_entity)
...
class TestCalcRequestHandler:
    ...
    def sample_event(self):
        ...
        data = TestDataBuilder.cloud_event_data(
            processor_name="test_processor", entity_id="test-entity-123"
        )
        return create_mock_cloud_event(
            event_type=CALC_REQ_EVENT_TYPE, event_id="calc-event-123", data=data
        )
    ...
    async def test_processor_failure(self, handler, mock_services, sample_event):
        ...
        mock_services.processor_manager.add_processor(
            "test_processor", failing_processor
        )
    ...
    async def test_invalid_entity_type(self, handler, mock_services):
        ...
        event = create_mock_cloud_event(event_type=CALC_REQ_EVENT_TYPE, data=data)
...
class TestCriteriaCalcRequestHandler:
    ...
    def sample_event(self):
        ...
        data = TestDataBuilder.cloud_event_data(entity_id="test-entity-123")
    ...
    @pytest.mark.asyncio
    async def test_successful_criteria_check(
        self, handler, mock_services, sample_event
    ):
...
class TestErrorHandler:
    ...
    async def test_error_with_missing_fields(self, handler):
        ...
        event = create_mock_cloud_event(
            event_type="error", data={}  # Missing required fields
        )
...
class TestHandlerIntegration:
    ...
    async def test_handler_chain_processing(self):
        ...
        criteria_event = create_mock_cloud_event(
            event_type=CRITERIA_CALC_REQ_EVENT_TYPE, data=criteria_data
        )
