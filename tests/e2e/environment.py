...
def before_all(context):
    ...
    logger.info(
        f"Config was ehanced. Added new processor module: {test_processor_module}"
    )
    config["processor"]["modules"].append(test_criterion_module)
    logger.info(
        f"Config was ehanced. Added new criterion module: {test_criterion_module}"
    )
    ...
    context.loop_thread = threading.Thread(
        target=run_loop, args=(context.loop,), daemon=True
    )
...
def after_scenario(context, scenario):
    ...
    if hasattr(context, "entity_service"):
        cleanup_future = asyncio.run_coroutine_threadsafe(
            context.entity_service.delete_all("nobel-prize", "1"), context.loop
        )
        try:
