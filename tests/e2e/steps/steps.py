...
def i_create_the_prizes_in_bulk(context):
    context.create_result = run(
        context.entity_service.save_all(
            context.prizes_to_create, entity_class="nobel-prize", entity_version="1"
        )
    )

    def await_created():
        search_result = run(context.entity_service.find_all("nobel-prize", "1"))
        return len(search_result) > 0

    assert poll_for_condition(
        await_created
    ), "Didn't got confirmation that entities succesfully saved"


@when("I get all of model {model_name} version {model_version}")
def i_get_all_of_model(context, model_name, model_version):
    model_name = model_name.replace('"', "")
    context.search_result = run(
        context.entity_service.find_all(model_name, model_version)
    )


@when("I get the prize by its ID")
def i_get_prize_by_id(context):
    context.get_by_id = run(
        context.entity_service.get_by_id(
...
@when("I update the prize with the year {year} and transition {transition_name}")
def i_update_prize_year_with_transition(context, year, transition_name):
    ...
    context.update_result = run(
        context.entity_service.update(
...
def i_delete_the_prize_by_its_id(context):
    run(
        context.entity_service.delete_by_id(
...
@when("I delete all of model {model_name} version {model_version}")
def i_delete_all_models(context, model_name, model_version):
    model_name = model_name.replace('"', "")
    context.deleted_count = run(
        context.entity_service.delete_all(model_name, model_version)
    )


@when("I fetching of models {model_name} version {model_version} by condition:")
def fetching_by_condition(context, model_name, model_version):
    model_name = model_name.replace('"', "")
    condition = json.loads(context.text)["conditions"][0]
    context.search_result = run(
        context.entity_service.search(
            model_name,
...
@when("I import workflow for model {model_name} version {model_version}:")
def import_workflow(context, model_name, model_version):
    model_name = model_name.replace('"', "")
    workflows = json.loads("[" + context.text + "]")
    context.workflow_import_result = run(
        context.workflow_management_service.import_entity_workflows(
            model_name,
            model_version,
            workflows,
            import_mode="REPLACE",
...
@when("I update the prize with transition {transition_name}")
...
@then("{expected_amount:d} prizes should be created successfully")
def prizes_should_be_created(context, expected_amount):
    ...
    ), f"Expected that {expected_amount} entities is created, but actually {actual_amount}"


@then("the prize's year should be {expected_year}")
def the_prizes_year_should_be(context, expected_year):
    ...
    ), f"Expected year: {expected_year}, but got {actual_year}"


@then("the update should be successful")
def the_update_should_be_successful(context):
    assert (
        context.update_result.data == context.create_result.data
...
def expected_length(context, expected_length):
    ...
    ), f"Expected length is {expected_length} but got {actual_length}"
...
def workflow_imported_succesfull(context):
    ...
    assert (
        is_success
    ), f"Expected that workflow imported successfuly but got an error: '{error}'"
...
def awaits_criterion_triggered_and_passed(context):
    ...
    assert (
        context.apply_transition_result
    ), "Expected that transition applied succesfully"
