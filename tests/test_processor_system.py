...
from common.processor.errors import (CriteriaError, CriteriaNotFoundError,
                                     ProcessorError, ProcessorNotFoundError)
...
class TestProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "test_processor", description: str = "Test processor"
    ):
...
class TestProcessorSystem:
    ...
    async def test_process_entity_success(self):
        ...
        result = await manager.process_entity(
            "test_processor", entity, test_param="custom_value"
        )
