...
class TestProcessorIntegration:
    ...
    def test_processor_manager_discovers_real_processors(self):
        ...
        for criteria_name in expected_criteria:
            assert (
                criteria_name in criteria
            ), f"Criteria '{criteria_name}' not discovered"
    ...
    async def test_criteria_checking_integration(self):
        ...
        if "entity_state" in criteria_list:
            ...
            result = await manager.check_criteria(
                "entity_state", entity, expected_states=["ACTIVE"]
            )
            assert result is True

            result2 = await manager.check_criteria(
                "entity_state", entity, expected_states=["INACTIVE"]
            )
            assert result2 is False
