...
if PYDANTIC_AVAILABLE:
    ...
    class EntityId(BaseValidatedModel):
        ...
        entity_id: str = Field(
            ..., description="Unique entity identifier", min_length=1, max_length=100
        )
    ...
    class ProcessorRequest(BaseValidatedModel):
        ...
        processor_name: constr(
            min_length=1, max_length=50, regex=r"^[a-zA-Z0-9_-]+$"
        ) = Field(..., description="Name of the processor to execute")
        ...
        timeout_seconds: confloat(gt=0, le=300) = Field(
            default=30, description="Processing timeout"
        )
        priority: conint(ge=1, le=10) = Field(
            default=5, description="Processing priority"
        )
    ...
    class ProcessorResponse(BaseValidatedModel):
        ...
        processor_name: str = Field(
            ..., description="Name of the processor that executed"
        )
    ...
    class CriteriaRequest(BaseValidatedModel):
        ...
        criteria_name: constr(
            min_length=1, max_length=50, regex=r"^[a-zA-Z0-9_-]+$"
        ) = Field(..., description="Name of the criteria to check")
        entity_id: constr(min_length=1, max_length=100) = Field(
    ...
    class CriteriaResponse(BaseValidatedModel):
        ...
        criteria_name: str = Field(
            ..., description="Name of the criteria that was checked"
        )
        entity_id: str = Field(..., description="ID of the checked entity")
        matches: bool = Field(
            ..., description="Whether the entity matches the criteria"
        )
        confidence: confloat(ge=0.0, le=1.0) = Field(
            default=1.0, description="Confidence score"
        )
        details: Dict[str, Any] = Field(
            default_factory=dict, description="Detailed check results"
        )
        processing_time_ms: confloat(ge=0) = Field(
            ..., description="Check time in milliseconds"
        )
        metadata: Dict[str, Any] = Field(
            default_factory=dict, description="Response metadata"
        )
    ...
    class HealthCheckResult(BaseValidatedModel):
        ...
        duration_ms: confloat(ge=0) = Field(
    ...
    EntityMetadata = BaseValidatedModel  # type: ignore
    EntityId = BaseValidatedModel  # type: ignore
    ProcessorRequest = BaseValidatedModel  # type: ignore
    ProcessorResponse = BaseValidatedModel  # type: ignore
    CriteriaRequest = BaseValidatedModel  # type: ignore
    CriteriaResponse = BaseValidatedModel  # type: ignore
    HealthCheckResult = BaseValidatedModel  # type: ignore
    ConfigurationModel = BaseValidatedModel  # type: ignore


def validate_model_data(
    model_class: Type[BaseValidatedModel], data: Dict[str, Any]
) -> BaseValidatedModel:
