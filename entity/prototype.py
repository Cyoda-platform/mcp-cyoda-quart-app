...
email_open_stats = {
    "totalEmailsSent": 0,
    "totalOpens": 0,
}  # TODO: Track opens realistically (mocked here)

# Constants
CAT_FACT_API_URL = "https://catfact.ninja/fact"

# Helper function to send emails (mocked)
...
async def signup():
    try:
        ...
        if email in subscribers_cache:
            return jsonify({"message": "Email already subscribed", "subscriberId": subscribers_cache[email]["id"]}), 200

        subscriber_id = f"sub-{len(subscribers_cache) + 1}"
        subscribers_cache[email] = {"id": subscriber_id, "email": email, "subscribedAt": datetime.utcnow().isoformat()}
...
async def fetch_cat_fact() -> str:
    async with httpx.AsyncClient() as client:
        try:
            ...
            fact = data.get("fact")
            if not fact:
                raise ValueError("No fact found in API response")
            return fact
        except Exception as e:
            logger.exception(e)
...
async def process_send_facts():
    ...
    email_subject = "Your Weekly Cat Fact 🐱"
    email_body = f"Hello!\n\nHere is your weekly cat fact:\n\n{fact}\n\nEnjoy your week!"

    send_tasks = []
    for subscriber in subscribers_cache.values():
        send_tasks.append(send_email(subscriber["email"], email_subject, email_body))

    # Fire and forget sending emails concurrently
    await asyncio.gather(*send_tasks)

    # Update stats
    email_open_stats["totalEmailsSent"] += len(subscribers_cache)
    # TODO: Open tracking is mocked and not implemented here
...
async def report_interactions():
    total_sent = email_open_stats.get("totalEmailsSent", 0)
    total_opens = email_open_stats.get("totalOpens", 0)
    open_rate = (total_opens / total_sent) if total_sent > 0 else 0
    return jsonify({
        "totalEmailsSent": total_sent,
        "totalOpens": total_opens,
        "openRate": round(open_rate, 3),
    })
