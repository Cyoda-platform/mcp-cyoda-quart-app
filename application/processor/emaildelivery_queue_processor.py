...
class EmailDeliveryQueueProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "EmailDeliveryQueueProcessor", description: str = ""
    ):
        super().__init__(
            name=name, description=description or "Queues email for delivery"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            logger.info(
                f"Queued email delivery for subscriber {entity.subscriberId}, cat fact {entity.catFactId}"
            )
            return entity
            ...
            raise ProcessorError(
                self.name, f"Failed to queue email delivery: {str(e)}", e
            )
