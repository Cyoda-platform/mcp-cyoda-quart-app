...
class AppointmentStartProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "AppointmentStartProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Mark appointment as in progress"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            await self._verify_current_time_is_appointment_time(appointment.appointment_date)
            
            # Check pet and owner presence
            await self._check_pet_and_owner_presence(appointment.pet_id, appointment.owner_id)
            ...
            appointment.add_metadata("start_timestamp", datetime.now(timezone.utc).isoformat())
            
            logger.info(f"Successfully started appointment {appointment.appointment_id}")
    ...
    async def _verify_current_time_is_appointment_time(self, appointment_date: str) -> None:
        """Verify current time is appointment time."""
