...
class ReportGenerationProcessor(CyodaProcessor):
        ...
        try:
            ...
            for result in pet_results:
                if hasattr(result, 'data'):
                    pet_dict = result.data.model_dump() if hasattr(result.data, 'model_dump') else result.data
    ...
    async def _collect_store_data(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for result in store_results:
                if hasattr(result, 'data'):
                    store_dict = result.data.model_dump() if hasattr(result.data, 'model_dump') else result.data
    ...
    def _generate_report_data(
        self, report: Report, pet_data: List[Dict[str, Any]], store_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        overall_score = self._calculate_overall_performance(pet_analysis, store_analysis)
        ...
        report_data = {
            "report_metadata": {
                ...
                "data_sources": ["Pet Store API", "Cyoda Pet Entities", "Cyoda Store Entities"],
            },
            ...
            "key_metrics": {
                ...
                "average_store_score": store_analysis.get("average_performance_score", 0.0),
            },
    ...
    def _analyze_pets(self, pet_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_performing_pets": self._get_top_performers(pet_data, "performance_score"),
        }
    ...
    def _analyze_stores(self, store_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_inventory_per_store": total_inventory / total_stores if total_stores > 0 else 0,
            "performance_scores": performance_scores,
        }
    ...
    def _analyze_trends(
        ...
        return {
            ...
            "store_trends": {
                "average_availability_rate": self._calculate_avg_availability_rate(store_data),
                "average_sales_rate": self._calculate_avg_sales_rate(store_data),
            },
        }

    def _get_top_performers(self, data: List[Dict[str, Any]], score_field: str) -> List[Dict[str, Any]]:
        """Get top performing entities"""
        scored_items = [item for item in data if item.get(score_field, 0) > 0]
        sorted_items = sorted(scored_items, key=lambda x: x.get(score_field, 0), reverse=True)
    ...
    def _calculate_avg_availability_rate(self, store_data: List[Dict[str, Any]]) -> float:
...
The analysis covers inventory management, pet availability, and sales performance metrics.
    ...
    def _generate_recommendations(self, report_data: Dict[str, Any]) -> List[str]:
        ...
        if overall_score < 60.0:
            recommendations.append("Overall performance is below average - consider reviewing inventory and sales strategies")
        
        pet_analysis = report_data.get("pet_analysis", {})
        if pet_analysis.get("average_performance_score", 0.0) < 50.0:
            recommendations.append("Pet performance scores are low - focus on improving data completeness and availability")
        
        store_analysis = report_data.get("store_analysis", {})
        if store_analysis.get("average_performance_score", 0.0) < 50.0:
            recommendations.append("Store performance needs improvement - review inventory management processes")
        
        if overall_score >= 80.0:
            recommendations.append("Excellent performance - consider expanding operations or exploring new markets")
