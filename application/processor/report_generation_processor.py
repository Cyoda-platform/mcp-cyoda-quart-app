...
class ReportGenerationProcessor(CyodaProcessor):
    ...
    async def _fetch_analyzed_products(self) -> List[Product]:
        ...
        try:
            ...
            for result in results:
                try:
                    product_data = result.data.model_dump() if hasattr(result.data, 'model_dump') else result.data
    ...
    async def _generate_report_content(self, report: Report, products: List[Product]) -> None:
        ...
        report.summary = self._generate_executive_summary(products, top_products, underperforming, low_stock)
    ...
    def _identify_underperformers(self, products: List[Product]) -> List[Dict[str, Any]]:
        """Identify underperforming products that need attention"""
        underperformers = []
        
        for product in products:
            if (product.performance_score is not None and 
                product.performance_score < 30):
    ...
    def _identify_low_stock_products(self, products: List[Product]) -> List[Dict[str, Any]]:
    ...
    def _generate_executive_summary(
        ...
        avg_performance = sum(p.performance_score or 0 for p in products) / max(total_products, 1)
...
- Prioritize restocking for high-urgency items
    ...
    def _calculate_summary_metrics(self, report: Report, products: List[Product]) -> None:
