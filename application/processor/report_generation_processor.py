...
class ReportGenerationProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        try:
            ...
            self.logger.info(f"Retrieved {len(all_bookings)} bookings for report generation")

            # Apply filters if specified
            filtered_bookings = await self._apply_report_filters(all_bookings, report_entity.filter_criteria)
            ...
            if report_entity.report_type in ["date_range", "custom"]:
                date_range_stats = await self._generate_date_range_statistics(filtered_bookings)
                report_entity.date_range_stats = date_range_stats

            # Update report metadata
            report_entity.generated_at = datetime.now().isoformat().replace("+00:00", "Z")
    ...
    async def _apply_report_filters(self, bookings: List[Booking], filter_criteria: Dict[str, Any] | None) -> List[Booking]:
        ...
        for booking in bookings:
            try:
                ...
                self.logger.warning(f"Error applying filter to booking {booking.booking_id}: {str(e)}")
                continue
                
        self.logger.info(f"Filtered {len(bookings)} bookings down to {len(filtered_bookings)} based on criteria")
    ...
    def _generate_summary_statistics(self, bookings: List[Booking]) -> ReportSummary:
        ...
        average_booking_price = total_revenue / total_bookings if total_bookings > 0 else 0.0
        ...
        average_nights_per_booking = total_nights / bookings_with_nights if bookings_with_nights > 0 else 0.0
    ...
    def _analyze_booking_patterns(self, bookings: List[Booking]) -> Dict[str, Any]:
        ...
        try:
            ...
            for booking in bookings:
                try:
                    ...
                    if booking.bookingdates and booking.bookingdates.checkin:
                        checkin_date = datetime.strptime(booking.bookingdates.checkin, "%Y-%m-%d")
            ...
            if nights_count > 0:
                patterns["average_stay_duration"] = round(total_nights / nights_count, 2)
            
            # Find most common additional needs (top 3)
            if additional_needs:
                sorted_needs = sorted(additional_needs.items(), key=lambda x: x[1], reverse=True)
                patterns["most_common_additional_needs"] = [need for need, count in sorted_needs[:3]]
