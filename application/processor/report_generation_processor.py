...
class ReportGenerationProcessor(CyodaProcessor):
    ...
    async def _get_analyzed_products(self) -> List[Product]:
        ...
        try:
            ...
            for response in product_responses:
                ...
                if product.state in ["analyzed", "completed"] and product.performance_score is not None:
                    products.append(product)
            
            self.logger.info(f"Retrieved {len(products)} analyzed products for report generation")
            return products
    ...
    async def _generate_report_content(self, report: Report, products: List[Product]) -> str:
        """
    ...
    def _generate_executive_summary(self, products: List[Product]) -> str:
        ...
        avg_performance = sum(p.performance_score or 0 for p in products) / len(products)
    ...
    def _generate_performance_overview(self, products: List[Product]) -> str:
        ...
        performance_ranges = {
            "Excellent (80-100)": len([p for p in products if (p.performance_score or 0) >= 80]),
            "Good (60-79)": len([p for p in products if 60 <= (p.performance_score or 0) < 80]),
            "Average (40-59)": len([p for p in products if 40 <= (p.performance_score or 0) < 60]),
            "Below Average (20-39)": len([p for p in products if 20 <= (p.performance_score or 0) < 40]),
            "Poor (0-19)": len([p for p in products if (p.performance_score or 0) < 20])
        }
    ...
    def _generate_top_performers_section(self, products: List[Product]) -> str:
        ...
        section = ["| Product | Category | Performance Score | Revenue | Sales Volume |", 
                  "|---------|----------|------------------|---------|--------------|"]
    ...
    def _generate_underperformers_section(self, products: List[Product]) -> str:
        ...
        section = ["| Product | Category | Performance Score | Revenue | Issues |",
                  "|---------|----------|------------------|---------|--------|"]
    ...
    def _generate_category_analysis(self, products: List[Product]) -> str:
        ...
        section = ["| Category | Products | Total Revenue | Total Sales | Avg Performance |",
                  "|----------|----------|---------------|-------------|-----------------|"]
    ...
    def _generate_recommendations(self, products: List[Product]) -> str:
        ...
        if out_of_stock_count > 0:
            recommendations.append(f"🚨 **Immediate Action Required:** {out_of_stock_count} products are out of stock and need immediate restocking.")
        
        low_stock_count = len([p for p in products if p.stock_status == "LOW_STOCK"])
        if low_stock_count > 0:
            recommendations.append(f"⚠️ **Plan Restocking:** {low_stock_count} products have low inventory levels.")
        
        # Performance recommendations
        high_performers = [p for p in products if p.is_high_performer()]
        if high_performers:
            recommendations.append(f"✅ **Expand Successful Products:** Consider increasing inventory for {len(high_performers)} high-performing products.")
        
        low_performers = [p for p in products if p.is_low_performer()]
        if low_performers:
            recommendations.append(f"📊 **Review Strategy:** {len(low_performers)} products need marketing or pricing strategy review.")
        ...
        for category, scores in category_performance.items():
            ...
            if avg_score >= 70:
                recommendations.append(f"🎯 **{category} Category:** Strong performance - consider expanding product line.")
            elif avg_score <= 30:
                recommendations.append(f"🔍 **{category} Category:** Underperforming - review product mix and pricing.")
    ...
    def _generate_insights(self, products: List[Product]) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_performers_count": len([p for p in products if p.is_high_performer()]),
    ...
    def _get_top_category(self, products: List[Product]) -> str:
        ...
        return max(category_averages, key=category_averages.get) if category_averages else "N/A"
