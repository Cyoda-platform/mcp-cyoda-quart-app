...
class ReportGenerationProcessor(CyodaProcessor):
    ...
    async def _populate_report_data(
        ...
        for product in sorted(
            top_performers, key=lambda x: x.performance_score or 0, reverse=True
        )[:5]:
        ...
        for product in sorted(underperformers, key=lambda x: x.performance_score or 0)[
            :5
        ]:
            ...
            report.add_restock_recommendation(
                {
                    ...
                    "sales_volume": product.sales_volume,
                    "urgency": (
                        "high" if (product.inventory_level or 0) < 5 else "medium"
                    ),
                }
            )
    ...
    def _generate_executive_summary(self, report: Report) -> None:
        ...
        if restock_count > 0:
            ...
            summary_parts.append(
                "Overall positive performance trend - consider expanding successful products."
            )
        else:
            summary_parts.append(
                "Mixed performance - focus on improving underperforming products."
            )
