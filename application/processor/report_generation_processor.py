...
class ReportGenerationProcessor(CyodaProcessor):
    ...
    async def _fetch_products_for_analysis(self) -> List[Product]:
        ...
        try:
            ...
            for response in product_responses:
                try:
                    product_data = response.data.model_dump() if hasattr(response.data, 'model_dump') else response.data
    ...
    async def _generate_report_content(self, report: Report, products: List[Product]) -> None:
        """
        ...
        try:
            ...
            content_parts.append(f"**Report Period:** {report.report_period_start} to {report.report_period_end}")
            content_parts.append(f"**Generated:** {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}")
            ...
            content_parts.append(f"- **Total Products Analyzed:** {stats['total_products']}")
            content_parts.append(f"- **Total Revenue:** ${stats['total_revenue']:,.2f}")
            content_parts.append(f"- **Average Performance Score:** {stats['avg_performance_score']:.1f}/100")
            content_parts.append(f"- **High Performers:** {stats['high_performers_count']} products")
            content_parts.append(f"- **Products Needing Restock:** {stats['restock_needed_count']} products")
            content_parts.append("")
            ...
            if stats['top_performers']:
                ...
                for i, product in enumerate(stats['top_performers'][:5], 1):
                    content_parts.append(f"{i}. **{product.name}** (Score: {product.performance_score:.1f})")
                    content_parts.append(f"   - Category: {product.category}")
                    content_parts.append(f"   - Revenue: ${product.revenue or 0:.2f}")
                    content_parts.append(f"   - Sales Volume: {product.sales_volume or 0}")
                content_parts.append("")
            ...
            if stats['underperformers']:
                ...
                for i, product in enumerate(stats['underperformers'][:5], 1):
                    ...
                    if product.needs_restocking():
                        content_parts.append(f"   - ⚠️ **Needs Restocking** (Inventory: {product.inventory_level})")
                content_parts.append("")
    ...
    def _generate_executive_summary(self, stats: Dict[str, Any]) -> str:
        ...
        summary_parts.append(f"This weekly performance report analyzes {stats['total_products']} products "
                           f"with a combined revenue of ${stats['total_revenue']:,.2f}.")
        
        if stats['high_performers_count'] > 0:
            summary_parts.append(f"We identified {stats['high_performers_count']} high-performing products "
                               f"that are driving strong sales.")
        
        if stats['restock_needed_count'] > 0:
            summary_parts.append(f"**Action Required:** {stats['restock_needed_count']} products "
                               f"require immediate restocking to avoid stockouts.")
        
        if stats['avg_performance_score'] >= 70:
            summary_parts.append("Overall product performance is strong with good sales momentum.")
        elif stats['avg_performance_score'] >= 50:
            summary_parts.append("Product performance is moderate with opportunities for improvement.")
        else:
            summary_parts.append("Product performance needs attention with several underperforming items.")
    ...
    def _calculate_summary_statistics(self, products: List[Product]) -> Dict[str, Any]:
        ...
        performance_scores = [p.performance_score for p in products if p.performance_score is not None]
        ...
        underperformers = [p for p in products if p.performance_score is not None and p.performance_score < 40]
        restock_needed = [p for p in products if p.needs_restocking()]
        
        # Sort by performance score
        top_performers = sorted(products, key=lambda p: p.performance_score or 0, reverse=True)
    ...
    def _calculate_category_statistics(self, products: List[Product]) -> Dict[str, Dict[str, Any]]:
        """
    ...
    def _generate_insights(self, report: Report, products: List[Product]) -> None:
        ...
        # Category insights
        category_stats = self._calculate_category_statistics(products)
        best_category = max(category_stats.items(), key=lambda x: x[1]['avg_score']) if category_stats else None
        if best_category:
            insights.append({
                "type": "insight",
    ...
    def _set_report_metadata(self, report: Report, products: List[Product]) -> None:
        ...
        # Set product lists
        report.top_performing_products = [p.technical_id or p.entity_id for p in stats['top_performers'][:10]]
        report.underperforming_products = [p.technical_id or p.entity_id for p in stats['underperformers'][:10]]
        report.products_needing_restock = [p.technical_id or p.entity_id for p in products if p.needs_restocking()]
