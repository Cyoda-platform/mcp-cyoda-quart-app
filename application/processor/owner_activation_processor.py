...
class OwnerActivationProcessor(CyodaProcessor):
    ...
    def __init__(self):
        super().__init__(
            name="OwnerActivationProcessor",
            description="Reactivates owner account"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["inactive"]:
                raise ProcessorError(self.name, f"Owner must be inactive for activation, current state: {entity.state}")
            ...
            entity.add_metadata("reactivated_at", datetime.now(timezone.utc).isoformat())
            ...
            logger.exception(f"Failed to process owner activation for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process owner activation: {str(e)}", e)
