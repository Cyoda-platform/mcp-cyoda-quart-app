...
class ValidateHnItemProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        try:
            ...
            if validation_errors:
                ...
                self.logger.info(
                    f"HnItem {hn_item.id} validation passed successfully"
                )
    ...
    async def _validate_item(self, item: HnItem) -> list[str]:
        ...
        if not item.type or item.type not in HnItem.ALLOWED_TYPES:
            validation_errors.append(f"Invalid or missing type. Must be one of: {HnItem.ALLOWED_TYPES}")
    ...
    async def _validate_type_specific_rules(self, item: HnItem, validation_errors: list[str]) -> None:
        """Validate type-specific business rules"""
        if item.type == "comment":
            # Comments should have a parent (but we'll be lenient for top-level comments)
            if item.parent is None:
                self.logger.info(f"Comment {item.id} has no parent - may be a top-level comment")
            ...
            if not item.title:
                validation_errors.append(f"{item.type.capitalize()}s should have a title")

    async def _validate_relationships(self, item: HnItem, validation_errors: list[str]) -> None:
        ...
        try:
            ...
            if item.parent:
                ...
                if not parent_exists:
                    validation_errors.append(f"Parent item {item.parent} does not exist")
            ...
            if item.parts:
                for part_id in item.parts:
                    ...
                    if not part_exists:
                        validation_errors.append(f"Poll option {part_id} does not exist")

        except Exception as e:
            self.logger.warning(f"Error validating relationships for item {item.id}: {str(e)}")
            # Don't fail validation for relationship check errors, just log them

    def _validate_business_rules(self, item: HnItem, validation_errors: list[str]) -> None:
        """Validate business rules"""
        # Only certain types can have scores
        if item.score is not None and item.type not in ["story", "poll", "job", "pollopt"]:
        ...
        if item.parent is not None and item.type != "comment":
            validation_errors.append(f"Only comments can have parents, but {item.type} has parent {item.parent}")

        # Only poll options can reference polls
        if item.poll is not None and item.type != "pollopt":
            validation_errors.append(f"Only poll options can reference polls, but {item.type} references poll {item.poll}")

        # Only polls can have parts
        if item.parts is not None and item.type != "poll":
            validation_errors.append(f"Only polls can have parts, but {item.type} has parts")

        # Validate source
        if item.source and item.source not in HnItem.ALLOWED_SOURCES:
            validation_errors.append(f"Invalid source. Must be one of: {HnItem.ALLOWED_SOURCES}")
