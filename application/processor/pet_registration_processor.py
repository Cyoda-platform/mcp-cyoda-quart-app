...
class PetRegistrationProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "PetRegistrationProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Register a new pet in the system"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if not pet.registration_date:
                pet.registration_date = datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')
            ...
            pet.add_metadata("registration_timestamp", datetime.now(timezone.utc).isoformat())
            
            logger.info(f"Successfully registered pet {pet.pet_id} for owner {pet.owner_id}")
