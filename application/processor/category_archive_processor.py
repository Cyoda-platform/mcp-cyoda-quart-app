...
class CategoryArchiveProcessor(CyodaProcessor):
    ...
    def __init__(self):
        super().__init__(
            name="CategoryArchiveProcessor", description="Archives category permanently"
        )
        ...
        try:
            ...
            if entity.state not in ["inactive"]:
                raise ProcessorError(
                    self.name,
                    f"Category must be inactive for archiving, current state: {entity.state}",
                )
            ...
            logger.exception(
                f"Failed to process category archiving for entity {entity.entity_id}"
            )
            raise ProcessorError(
                self.name, f"Failed to process category archiving: {str(e)}", e
            )
