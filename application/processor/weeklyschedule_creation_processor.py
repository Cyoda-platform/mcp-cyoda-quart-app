...
class WeeklyScheduleCreationProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "WeeklyScheduleCreationProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Creates weekly schedule for cat fact distribution"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if not entity.scheduledDate:
                ...
                entity.scheduledDate = datetime.combine(next_monday, datetime.min.time().replace(hour=9))
            ...
            active_count = sum(1 for sub in subscribers if sub.data.get("isActive") and sub.data.get("state") == "active")
            entity.subscriberCount = active_count
            
            logger.info(f"Created weekly schedule for {entity.weekStartDate} with {active_count} subscribers")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to create weekly schedule {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to create weekly schedule: {str(e)}", e)
