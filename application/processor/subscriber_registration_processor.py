...
class SubscriberRegistrationProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "SubscriberRegistrationProcessor", description: str = ""
    ):
        super().__init__(
            name=name,
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            for existing in existing_subscribers:
                if existing.data.get("email") == entity.email:
                    if existing.data.get("isActive"):
                        raise ProcessorError(
                            self.name, "Email already subscribed and active"
                        )
                    else:
            ...
            raise ProcessorError(
                self.name, f"Failed to register subscriber: {str(e)}", e
            )
