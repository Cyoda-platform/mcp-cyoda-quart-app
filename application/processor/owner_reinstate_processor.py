    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["suspended"]:
                raise ProcessorError(self.name, f"Owner must be suspended for reinstatement, current state: {entity.state}")
            ...
            logger.exception(f"Failed to process owner reinstatement for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process owner reinstatement: {str(e)}", e)
