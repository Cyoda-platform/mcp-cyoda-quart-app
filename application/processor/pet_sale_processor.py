    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["pending"]:
                raise ProcessorError(self.name, f"Pet must be pending for sale, current state: {entity.state}")
            ...
            logger.info(f"Successfully processed sale for pet {entity.name} to owner {entity.ownerId}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to process pet sale for entity {entity.entity_id}")
