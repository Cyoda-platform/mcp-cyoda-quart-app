...
class AppointmentNoShowProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "AppointmentNoShowProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Mark appointment as no-show"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            appointment.add_metadata("no_show_timestamp", datetime.now(timezone.utc).isoformat())
            ...
            logger.exception(f"Failed to mark appointment as no-show {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to mark appointment as no-show: {str(e)}", e)
    ...
    async def _apply_no_show_policy(self, appointment: Appointment) -> None:
        """Apply no show policy."""
        logger.info(f"Applying no show policy for appointment {appointment.appointment_id}")
        # TODO: Implement actual policy application
    
    async def _notify_vet_of_no_show(self, vet_id: str, appointment: Appointment) -> None:
        """Notify vet of no show."""
