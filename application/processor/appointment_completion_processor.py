...
class AppointmentCompletionProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "AppointmentCompletionProcessor", description: str = ""
    ):
        super().__init__(
            name=name, description=description or "Complete an appointment"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            logger.info(
                f"Successfully completed appointment {appointment.appointment_id}"
            )
            ...
            raise ProcessorError(
                self.name, f"Failed to complete appointment: {str(e)}", e
            )
    ...
    async def _calculate_actual_duration(self, appointment: Appointment) -> int:
        """Calculate actual duration."""
        logger.info(
            f"Calculating actual duration for appointment {appointment.appointment_id}"
        )
    ...
    async def _create_medical_record(self, appointment: Appointment) -> None:
        ...
        logger.info(
            f"Creating medical record for appointment {appointment.appointment_id}"
        )
