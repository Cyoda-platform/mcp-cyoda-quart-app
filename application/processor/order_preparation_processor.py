    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["confirmed"]:
                raise ProcessorError(self.name, f"Order must be confirmed for preparation, current state: {entity.state}")
            
            # Schedule delivery date (3-5 days from now)
            delivery_date = (datetime.now(timezone.utc) + timedelta(days=4)).date().isoformat()
            ...
            entity.add_metadata("preparation_started", datetime.now(timezone.utc).isoformat())
            ...
            logger.info(f"Successfully started preparation for order {entity.entity_id}, delivery scheduled for {delivery_date}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to process order preparation for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process order preparation: {str(e)}", e)
