...
class WeeklyScheduleEmailDistributionProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "WeeklyScheduleEmailDistributionProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Initiates email distribution to all subscribers"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if not entity.catFactId:
                raise ProcessorError(self.name, "WeeklySchedule must have catFactId to distribute emails")
            ...
            try:
                catfact_response = await entity_service.get_by_id(str(entity.catFactId), "catfact", "1")
                ...
                raise ProcessorError(self.name, f"Failed to trigger cat fact distribution: {str(e)}")
            
            logger.info(f"Initiated email distribution for weekly schedule {entity.id}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to initiate email distribution for weekly schedule {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to initiate email distribution: {str(e)}", e)
