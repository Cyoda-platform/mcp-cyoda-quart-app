    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["delivered"]:
                raise ProcessorError(self.name, f"Order must be delivered for return, current state: {entity.state}")
            ...
            logger.info(f"Successfully processed return for order {entity.entity_id}, reason: {return_reason}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to process order return for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process order return: {str(e)}", e)
