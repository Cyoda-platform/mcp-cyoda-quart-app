...
class PerformanceReportGenerationProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        try:
            ...
            report.calculate_summary_metrics(
                [p.model_dump(by_alias=True) for p in product_data_list]
            )
    ...
    async def _fetch_product_data(self) -> List[ProductData]:
        ...
        try:
            ...
            self.logger.info(
                f"Fetched {len(product_data_list)} product data entities for analysis"
            )
    ...
    async def _generate_executive_summary(
        self, report: PerformanceReport, product_data_list: List[ProductData]
    ) -> None:
        """
        ...
        items_needing_restock = len(
            [p for p in product_data_list if p.requires_restocking]
        )
        ...
        self.logger.info(
            f"Generated executive summary with {total_products} products analyzed"
        )
    ...
    async def _analyze_sales_trends(
        ...
        sorted_by_sales = sorted(
            product_data_list, key=lambda p: p.sales_volume or 0, reverse=True
        )
    ...
    async def _analyze_inventory_status(
        self, report: PerformanceReport, product_data_list: List[ProductData]
    ) -> None:
        """
        ...
        for product in restock_items:
            restock_info = {
                ...
                "urgency": (
                    "HIGH"
                    if (product.stock_level or 0) < 5
                    else "MEDIUM" if (product.stock_level or 0) < 10 else "LOW"
                ),
            }
    ...
    async def _generate_insights_and_recommendations(
        ...
        if high_performers / total_products < 0.2:
            report.add_recommendation(
                "Consider reviewing pricing strategy and marketing efforts to improve overall product performance"
            )

        if restock_needed > total_products * 0.15:
            report.add_recommendation(
                "Implement automated inventory monitoring to prevent stockouts of popular items"
            )

        if slow_moving > total_products * 0.25:
            report.add_recommendation(
                "Review slow-moving inventory for potential promotions or discontinuation"
            )
        ...
        if category_stats:
            best_category = max(
                ...
                key=lambda x: x[1]["high_performers"] / x[1]["count"],
            )
            report.add_recommendation(
                f"Focus marketing efforts on {best_category[0]} category which shows highest performance rate"
            )

        self.logger.info(
            f"Generated {len(report.performance_trends)} trends and {len(report.recommendations)} recommendations"
        )
