...
class MedicalRecordCompletionProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "MedicalRecordCompletionProcessor", description: str = ""
    ):
        super().__init__(
            name=name, description=description or "Complete a medical record"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            raise ProcessorError(
                self.name, f"Failed to complete medical record: {str(e)}", e
            )
    ...
    async def _notify_owner_of_record(self, pet_id: str, record: MedicalRecord) -> None:
        """Notify owner of record."""
        logger.info(
            f"Notifying owner of medical record {record.record_id} for pet {pet_id}"
        )
        # TODO: Get owner through pet and send notification

    async def _update_pet_medical_history(
        self, pet_id: str, record: MedicalRecord
    ) -> None:
        """Update pet medical history (no transition)."""
