    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["available"]:
                raise ProcessorError(self.name, f"Pet must be available for reservation, current state: {entity.state}")
            ...
            logger.exception(f"Failed to process pet reservation for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process pet reservation: {str(e)}", e)
