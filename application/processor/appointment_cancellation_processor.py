...
class AppointmentCancellationProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "AppointmentCancellationProcessor", description: str = ""
    ):
        super().__init__(name=name, description=description or "Cancel an appointment")
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            # Notify all parties
            await self._notify_all_parties(
                appointment.owner_id, appointment.vet_id, appointment
            )
            ...
            appointment.add_metadata(
                "cancellation_reason", kwargs.get("reason", "Manual cancellation")
            )

            logger.info(
                f"Successfully cancelled appointment {appointment.appointment_id}"
            )
            ...
            raise ProcessorError(
                self.name, f"Failed to cancel appointment: {str(e)}", e
            )
    ...
    async def _notify_all_parties(
        self, owner_id: str, vet_id: str, appointment: Appointment
    ) -> None:
        """Notify all parties."""
        logger.info(
            f"Notifying all parties for appointment {appointment.appointment_id}"
        )
    ...
    async def _apply_cancellation_policy(self, appointment: Appointment) -> None:
        """Apply cancellation policy."""
        logger.info(
            f"Applying cancellation policy for appointment {appointment.appointment_id}"
        )
