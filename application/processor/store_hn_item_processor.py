...
class StoreHnItemProcessor(CyodaProcessor):
    ...
    async def _update_existing_item(self, new_item: HnItem, existing_item: Any) -> None:
        ...
        try:
            ...
            self.logger.info(f"Updated existing item {existing_id} with HN ID {new_item.id}")
    ...
    async def _update_relationships(self, item: HnItem) -> None:
        ...
        try:
            ...
            if item.kids:
                self.logger.info(f"Item {item.id} has {len(item.kids)} children: {item.kids}")
            
            if item.poll:
                self.logger.info(f"Poll option {item.id} belongs to poll {item.poll}")
            
            if item.parts:
                self.logger.info(f"Poll {item.id} has {len(item.parts)} options: {item.parts}")
                
        except Exception as e:
            self.logger.warning(f"Error updating relationships for item {item.id}: {str(e)}")
