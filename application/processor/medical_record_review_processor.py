...
class MedicalRecordReviewProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "MedicalRecordReviewProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Review a medical record"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            record.add_metadata("review_timestamp", datetime.now(timezone.utc).isoformat())
            ...
            raise ProcessorError(self.name, f"Failed to review medical record: {str(e)}", e)
