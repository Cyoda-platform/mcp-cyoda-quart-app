...
class BookingDataProcessor(CyodaProcessor):
    ...
    async def _enrich_booking_data(self, booking_entity: Booking) -> None:
        ...
        try:
            ...
            booking_entity.retrieved_at = datetime.now().isoformat().replace("+00:00", "Z")
            ...
            self.logger.info(f"Enriched booking data for booking {booking_entity.booking_id}")
            
        except Exception as e:
            self.logger.error(f"Error enriching booking data: {str(e)}")
            # Don't raise - enrichment is optional

    async def _apply_filters(self, booking_entity: Booking, filter_criteria: Dict[str, Any]) -> None:
    ...
    async def _get_bookings_in_date_range(self, start_date: str, end_date: str) -> List[Booking]:
