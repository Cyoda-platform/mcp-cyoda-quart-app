...
class AppointmentSchedulingProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "AppointmentSchedulingProcessor", description: str = ""
    ):
        super().__init__(
            name=name, description=description or "Schedule a new appointment"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            await self._check_scheduling_conflicts(
                appointment.vet_id, appointment.appointment_date
            )
            ...
            raise ProcessorError(
                self.name, f"Failed to schedule appointment: {str(e)}", e
            )
    ...
    async def _check_scheduling_conflicts(
        self, vet_id: str, appointment_date: str
    ) -> None:
        """Check for scheduling conflicts."""
        logger.debug(
            f"Checking scheduling conflicts for vet {vet_id} on {appointment_date}"
        )
