        ...
        try:
            ...
            if entity.state not in ["active"]:
                raise ProcessorError(
                    self.name,
                    f"Owner must be active for suspension, current state: {entity.state}",
                )
            ...
            logger.info(
                f"Successfully suspended owner {entity.email}, reason: {suspension_reason}"
            )
            return entity

        except Exception as e:
            logger.exception(
                f"Failed to process owner suspension for entity {entity.entity_id}"
            )
            raise ProcessorError(
                self.name, f"Failed to process owner suspension: {str(e)}", e
            )
