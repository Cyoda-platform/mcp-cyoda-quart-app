    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["active"]:
                raise ProcessorError(self.name, f"Category must be active for deactivation, current state: {entity.state}")
            ...
            logger.exception(f"Failed to process category deactivation for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process category deactivation: {str(e)}", e)
