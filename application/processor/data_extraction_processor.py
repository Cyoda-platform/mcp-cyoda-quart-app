...
class DataExtractionProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        try:
            ...
            if extracted_data:
                ...
                self.logger.error(f"Data extraction {extraction.technical_id} failed - no data retrieved")
    ...
    async def _extract_from_api(self, extraction: DataExtraction) -> Optional[List[Dict[str, Any]]]:
        """
        Extract data from Pet Store API.
        ...
        try:
            ...
            self.logger.info(f"Successfully extracted {len(simulated_data)} products from API")
    ...
    async def _create_product_entities(self, extracted_data: List[Dict[str, Any]]) -> None:
        """
        Create Product entities from extracted data.
        ...
        for product_data in extracted_data:
            try:
                ...
                self.logger.debug(f"Created Product entity {created_product_id} for {product.product_id}")
                
            except Exception as e:
                self.logger.error(f"Failed to create Product entity for {product_data.get('id', 'unknown')}: {str(e)}")
                continue
        
        self.logger.info(f"Successfully created {created_count} Product entities from extracted data")
    ...
    def _transform_to_product_entity(self, api_data: Dict[str, Any]) -> Product:
        ...
        if price is None:
            ...
            category_prices = {
                "dog": 50.0, "cat": 40.0, "bird": 30.0,
                "fish": 25.0, "reptile": 60.0, "small-pet": 35.0
            }
