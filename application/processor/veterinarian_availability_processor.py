...
class VeterinarianAvailabilityProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "VeterinarianAvailabilityProcessor", description: str = ""
    ):
        super().__init__(
            name=name, description=description or "Mark veterinarian as available again"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            vet.add_metadata(
                "availability_timestamp", datetime.now(timezone.utc).isoformat()
            )
            ...
            logger.exception(
                f"Failed to mark veterinarian available {entity.entity_id}"
            )
            raise ProcessorError(
                self.name, f"Failed to mark veterinarian available: {str(e)}", e
            )
