...
class EmailDeliveryOpenProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "EmailDeliveryOpenProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Tracks email opening"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state != "delivered":
                raise ProcessorError(self.name, f"Email must be delivered before tracking open, current state: {entity.state}")
