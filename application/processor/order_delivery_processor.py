    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["shipped"]:
                raise ProcessorError(self.name, f"Order must be shipped for delivery, current state: {entity.state}")
            
            # Get delivery confirmation
            delivery_confirmation = kwargs.get("deliveryConfirmation") or kwargs.get("delivery_confirmation", {})
            ...
            logger.exception(f"Failed to process order delivery for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process order delivery: {str(e)}", e)
