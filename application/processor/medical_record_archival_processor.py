...
class MedicalRecordArchivalProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "MedicalRecordArchivalProcessor", description: str = ""
    ):
        super().__init__(
            name=name, description=description or "Archive a medical record"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            raise ProcessorError(
                self.name, f"Failed to archive medical record: {str(e)}", e
            )
    ...
    async def _update_pet_medical_summary(
