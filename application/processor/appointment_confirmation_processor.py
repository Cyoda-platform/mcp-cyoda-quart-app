...
class AppointmentConfirmationProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "AppointmentConfirmationProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Confirm an appointment"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            appointment.add_metadata("confirmation_timestamp", datetime.now(timezone.utc).isoformat())
            
            logger.info(f"Successfully confirmed appointment {appointment.appointment_id}")
            
            return appointment
            
        except Exception as e:
            logger.exception(f"Failed to confirm appointment {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to confirm appointment: {str(e)}", e)
    
    def can_process(self, entity: CyodaEntity, **kwargs) -> bool:
        """Check if this processor can handle the entity."""
        return isinstance(entity, Appointment)
    
    async def _send_confirmation_to_owner(self, owner_id: str, appointment: Appointment) -> None:
        """Send confirmation to owner."""
        logger.info(f"Sending confirmation to owner {owner_id} for appointment {appointment.appointment_id}")
        # TODO: Implement actual confirmation sending
    
    async def _send_notification_to_vet(self, vet_id: str, appointment: Appointment) -> None:
        """Send notification to vet."""
        logger.info(f"Sending notification to vet {vet_id} for appointment {appointment.appointment_id}")
    ...
    async def _set_reminder_notifications(self, appointment: Appointment) -> None:
        """Set reminder notifications."""
        logger.info(f"Setting reminder notifications for appointment {appointment.appointment_id}")
