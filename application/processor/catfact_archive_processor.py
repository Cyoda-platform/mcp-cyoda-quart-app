...
class CatFactArchiveProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "CatFactArchiveProcessor", description: str = ""):
        super().__init__(
            name=name, description=description or "Archives cat fact after distribution"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state != "sent":
                raise ProcessorError(
                    self.name,
                    f"CatFact must be in sent state to archive, current state: {entity.state}",
                )

            # Add archival metadata
            entity.add_metadata("archived_at", entity.updated_at)
            entity.add_metadata(
                "archival_reason", "Automatic archival after distribution"
            )
