...
class VeterinarianHiringProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "VeterinarianHiringProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Process hiring of a new veterinarian"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if not vet.hire_date:
                vet.hire_date = datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')
            ...
            logger.info(f"Successfully hired veterinarian {vet.vet_id} with license {vet.license_number}")
