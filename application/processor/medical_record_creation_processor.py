...
class MedicalRecordCreationProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "MedicalRecordCreationProcessor", description: str = ""
    ):
        super().__init__(
            name=name, description=description or "Create a new medical record"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            raise ProcessorError(
                self.name, f"Failed to create medical record: {str(e)}", e
            )
