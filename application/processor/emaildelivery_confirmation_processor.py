...
class EmailDeliveryConfirmationProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "EmailDeliveryConfirmationProcessor", description: str = ""
    ):
        super().__init__(
            name=name, description=description or "Confirms email delivery"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if not entity.sentDate:
                raise ProcessorError(
                    self.name, "Email must be sent before confirmation"
                )

            # Add confirmation metadata
            entity.add_metadata("delivery_confirmed_at", entity.updated_at)
            entity.add_metadata(
                "confirmation_method", "Email service provider confirmation"
            )

            logger.info(
                f"Confirmed email delivery for subscriber {entity.subscriberId}"
            )
            return entity
            ...
            raise ProcessorError(
                self.name, f"Failed to confirm email delivery: {str(e)}", e
            )
