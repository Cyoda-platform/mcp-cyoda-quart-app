...
class OrderCancellationProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            logger.info(f"Successfully cancelled order {entity.entity_id}, reason: {cancellation_reason}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to process order cancellation for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process order cancellation: {str(e)}", e)
