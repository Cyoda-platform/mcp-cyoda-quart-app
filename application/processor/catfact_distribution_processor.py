...
class CatFactDistributionProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "CatFactDistributionProcessor", description: str = ""
    ):
        super().__init__(
            name=name,
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if not entity.scheduledSendDate:
                raise ProcessorError(
                    self.name, "CatFact must be scheduled before distribution"
                )

            if not entity.fact:
                raise ProcessorError(
                    self.name, "CatFact must have content for distribution"
                )
            ...
            for subscriber in active_subscribers:
                try:
                    ...
                    logger.error(
                        f"Failed to create email delivery for subscriber {subscriber.data.get('id')}: {e}"
                    )
                    continue
            ...
            raise ProcessorError(
                self.name, f"Failed to distribute cat fact: {str(e)}", e
            )
