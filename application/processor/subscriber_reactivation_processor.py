...
class SubscriberReactivationProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "SubscriberReactivationProcessor", description: str = ""):
        super().__init__(
            name=name,
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state != "bounced":
                raise ProcessorError(self.name, f"Subscriber must be in bounced state, current state: {entity.state}")
            ...
            raise ProcessorError(self.name, f"Failed to reactivate subscriber: {str(e)}", e)
