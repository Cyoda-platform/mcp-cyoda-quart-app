...
class EmailDeliveryBounceProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "EmailDeliveryBounceProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Handles permanent email bounce"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to trigger subscriber bounce for subscriber {entity.subscriberId}: {e}")
            
            logger.warning(f"Email bounced for subscriber {entity.subscriberId}: {bounce_reason}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to process email bounce {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process email bounce: {str(e)}", e)
