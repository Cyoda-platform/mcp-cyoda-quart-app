...
class AnalyzeDataProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        try:
            ...
            results, metrics = await self._perform_analysis(
    ...
    def _perform_statistical_analysis(
        ...
        if not df.empty:
            ...
            price_cols = [
                col for col in df.columns if "price" in col.lower() or "£" in col
            ]
            if price_cols:
    ...
    def _perform_trend_analysis(
        ...
        if not df.empty:
            ...
            if date_cols:
                ...
                metrics["date_range"] = {
                    "start": (
                        str(df[date_cols[0]].min())
                        if not df[date_cols[0]].isna().all()
                        else None
                    ),
                    "end": (
                        str(df[date_cols[0]].max())
                        if not df[date_cols[0]].isna().all()
                        else None
                    ),
                }
