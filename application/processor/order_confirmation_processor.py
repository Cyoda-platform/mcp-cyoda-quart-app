    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["placed"]:
                raise ProcessorError(self.name, f"Order must be placed for confirmation, current state: {entity.state}")
            ...
            logger.exception(f"Failed to process order confirmation for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process order confirmation: {str(e)}", e)
