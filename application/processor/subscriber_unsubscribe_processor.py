...
class SubscriberUnsubscribeProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "SubscriberUnsubscribeProcessor", description: str = ""
    ):
        super().__init__(
            name=name, description=description or "Processes subscriber unsubscription"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            raise ProcessorError(
                self.name, f"Failed to unsubscribe subscriber: {str(e)}", e
            )
