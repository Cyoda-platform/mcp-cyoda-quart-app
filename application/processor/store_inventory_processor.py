...
class StoreInventoryProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        try:
            ...
            performance_metrics = self._calculate_store_performance(store, inventory_data)
    ...
    async def _fetch_inventory_data(self) -> Dict[str, int]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(f"{self.pet_store_base_url}/store/inventory")
            ...
            self.logger.error(f"HTTP error fetching inventory: {e.response.status_code}")
    ...
    def _calculate_store_performance(self, store: Store, inventory_data: Dict[str, int]) -> Dict[str, Any]:
        """
        ...
        performance_metrics = {
            ...
            "rates": {
                ...
                "pending_rate": (pending_pets / total_pets * 100.0) if total_pets > 0 else 0.0,
            },
            "scores": {
    ...
    def _generate_recommendations(
        self, availability_rate: float, sales_rate: float, 
        available: int, pending: int, sold: int
    ) -> list[str]:
        """Generate recommendations based on store performance"""
        recommendations = []
        
        if availability_rate < 50.0:
            recommendations.append("Consider increasing available inventory to improve customer satisfaction")
        
        if sales_rate < 20.0:
            recommendations.append("Review pricing and marketing strategies to improve sales performance")
        
        if pending > available:
            recommendations.append("High pending inventory may indicate processing bottlenecks")
        
        if available == 0:
            recommendations.append("No available inventory - urgent restocking needed")
        
        if sales_rate > 80.0:
            recommendations.append("Excellent sales performance - consider expanding inventory")
        
        total = available + pending + sold
        if total < 10:
            recommendations.append("Low total inventory - consider expanding product range")
