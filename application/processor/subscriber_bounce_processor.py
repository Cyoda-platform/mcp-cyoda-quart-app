...
class SubscriberBounceProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "SubscriberBounceProcessor", description: str = ""):
        super().__init__(
            name=name, description=description or "Handles permanent email bounces"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            logger.exception(
                f"Failed to process bounce for subscriber {entity.entity_id}"
            )
            raise ProcessorError(
                self.name, f"Failed to process subscriber bounce: {str(e)}", e
            )
