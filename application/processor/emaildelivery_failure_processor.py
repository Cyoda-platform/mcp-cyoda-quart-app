...
class EmailDeliveryFailureProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "EmailDeliveryFailureProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Handles email delivery failure"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            logger.warning(f"Email delivery failed for subscriber {entity.subscriberId}: {failure_reason}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to process email delivery failure {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process email delivery failure: {str(e)}", e)
    ...
    def _is_permanent_failure(self, failure_reason: str) -> bool:
        ...
        return any(indicator in failure_reason.lower() for indicator in permanent_indicators)
