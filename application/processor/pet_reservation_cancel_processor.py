...
class PetReservationCancelProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            entity.add_metadata("reservation_cancelled_at", datetime.now(timezone.utc).isoformat())
            ...
            logger.info(f"Successfully cancelled reservation for pet {entity.name}, previous owner: {previous_owner}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to process pet reservation cancellation for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process pet reservation cancellation: {str(e)}", e)
