...
class SubscriberActivationProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "SubscriberActivationProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Activates subscriber after email verification or auto-activation"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["pending", "initial_state"]:
                raise ProcessorError(self.name, f"Subscriber must be in pending state, current state: {entity.state}")
            ...
            raise ProcessorError(self.name, f"Failed to activate subscriber: {str(e)}", e)
