        ...
        try:
            ...
            if entity.state not in ["active"]:
                raise ProcessorError(
                    self.name,
                    f"Owner must be active for deactivation, current state: {entity.state}",
                )
            ...
            logger.info(
                f"Successfully deactivated owner {entity.email}, reason: {reason}"
            )
            return entity

        except Exception as e:
            logger.exception(
                f"Failed to process owner deactivation for entity {entity.entity_id}"
            )
            raise ProcessorError(
                self.name, f"Failed to process owner deactivation: {str(e)}", e
            )
