...
class WeeklyScheduleCompletionProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "WeeklyScheduleCompletionProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Completes weekly schedule"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            logger.info(f"Completed weekly schedule {entity.id} for week {entity.weekStartDate}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to complete weekly schedule {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to complete weekly schedule: {str(e)}", e)
