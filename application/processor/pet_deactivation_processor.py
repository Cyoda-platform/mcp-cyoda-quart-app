...
class PetDeactivationProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "PetDeactivationProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Temporarily deactivate a pet"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            pet.add_metadata("deactivation_timestamp", datetime.now(timezone.utc).isoformat())
            pet.add_metadata("deactivation_reason", kwargs.get("reason", "Manual deactivation"))
    ...
    async def _notify_owner_of_deactivation(self, owner_id: str, pet_name: str) -> None:
        ...
        logger.info(f"Deactivation notification sent to owner {owner_id}: {notification_message}")
