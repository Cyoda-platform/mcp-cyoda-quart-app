...
class CatFactRetrievalProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "CatFactRetrievalProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Retrieves cat fact from external API"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            raise ProcessorError(self.name, f"Failed to retrieve cat fact from API: {str(e)}", e)
        except Exception as e:
            logger.exception(f"Failed to retrieve cat fact for entity {entity.entity_id}")
