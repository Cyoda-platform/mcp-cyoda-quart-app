...
class VeterinarianUnavailabilityProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "VeterinarianUnavailabilityProcessor", description: str = ""):
        super().__init__(
            name=name,
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            vet.add_metadata("unavailability_timestamp", datetime.now(timezone.utc).isoformat())
            vet.add_metadata("unavailability_reason", kwargs.get("reason", "Temporary unavailability"))
            ...
            logger.exception(f"Failed to mark veterinarian unavailable {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to mark veterinarian unavailable: {str(e)}", e)
    ...
    async def _update_schedule_availability(self, vet_id: str, available: bool) -> None:
        """Update schedule availability."""
        logger.info(f"Updating schedule availability for veterinarian {vet_id}: {available}")
