...
class VeterinarianLicenseVerificationProcessor(CyodaProcessor):
    ...
    def __init__(
        self,
        name: str = "VeterinarianLicenseVerificationProcessor",
        description: str = "",
    ):
        super().__init__(
            name=name, description=description or "Verify veterinary license"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            vet.add_metadata(
                "license_verification_date", datetime.now(timezone.utc).isoformat()
            )
            ...
            logger.exception(
                f"Failed to verify license for veterinarian {entity.entity_id}"
            )
