...
class PetAnalysisProcessor(CyodaProcessor):
    ...
    def _analyze_availability(self, pet: Pet) -> Dict[str, Any]:
        ...
        return {
            ...
            "status_category": "high" if availability_score >= 75 else "medium" if availability_score >= 50 else "low"
        }
    ...
    def _analyze_category_performance(self, pet: Pet) -> Dict[str, Any]:
        ...
        return {
            ...
            "category_tier": "premium" if category_score >= 80 else "standard" if category_score >= 60 else "basic"
        }
    ...
    def _analyze_data_completeness(self, pet: Pet) -> Dict[str, Any]:
        ...
        if pet.name:
            completeness_factors.append({"field": "name", "complete": True, "weight": 20})
            achieved_weight += 20
        else:
            completeness_factors.append({"field": "name", "complete": False, "weight": 20})
        total_weight += 20
        
        # Category (weight: 15)
        if pet.category:
            completeness_factors.append({"field": "category", "complete": True, "weight": 15})
            achieved_weight += 15
        else:
            completeness_factors.append({"field": "category", "complete": False, "weight": 15})
        total_weight += 15
        
        # Status (weight: 15)
        if pet.status:
            completeness_factors.append({"field": "status", "complete": True, "weight": 15})
            achieved_weight += 15
        else:
            completeness_factors.append({"field": "status", "complete": False, "weight": 15})
        total_weight += 15
        
        # Photo URLs (weight: 25)
        if pet.photo_urls and len(pet.photo_urls) > 0:
            completeness_factors.append({"field": "photo_urls", "complete": True, "weight": 25})
            achieved_weight += 25
        else:
            completeness_factors.append({"field": "photo_urls", "complete": False, "weight": 25})
        total_weight += 25
        
        # Tags (weight: 25)
        if pet.tags and len(pet.tags) > 0:
            completeness_factors.append({"field": "tags", "complete": True, "weight": 25})
            achieved_weight += 25
        else:
            completeness_factors.append({"field": "tags", "complete": False, "weight": 25})
        total_weight += 25
        
        completeness_score = (achieved_weight / total_weight * 100.0) if total_weight > 0 else 0.0
        ...
        return {
            ...
            "completeness_tier": "excellent" if completeness_score >= 90 else "good" if completeness_score >= 70 else "needs_improvement"
        }

    def _calculate_performance_score(self, pet: Pet, analysis_data: Dict[str, Any]) -> float:
        """
        ...
        availability_score = analysis_data.get("availability_analysis", {}).get("availability_score", 0.0)
        category_score = analysis_data.get("category_analysis", {}).get("category_score", 0.0)
        completeness_score = analysis_data.get("completeness_analysis", {}).get("completeness_score", 0.0)
