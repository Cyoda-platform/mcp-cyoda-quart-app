...
class VeterinarianTerminationProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "VeterinarianTerminationProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Process termination of veterinarian"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            vet.add_metadata("termination_timestamp", datetime.now(timezone.utc).isoformat())
            vet.add_metadata("termination_reason", kwargs.get("reason", "Employment termination"))
            ...
            raise ProcessorError(self.name, f"Failed to terminate veterinarian: {str(e)}", e)
