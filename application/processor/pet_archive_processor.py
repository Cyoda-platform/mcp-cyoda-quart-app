    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["sold"]:
                raise ProcessorError(self.name, f"Pet must be sold for archiving, current state: {entity.state}")
            ...
            logger.exception(f"Failed to process pet archiving for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process pet archiving: {str(e)}", e)
