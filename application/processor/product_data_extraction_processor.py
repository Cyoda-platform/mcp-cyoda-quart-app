...
class ProductDataExtractionProcessor(CyodaProcessor):
    ...
    async def _extract_pet_store_data(self, product: Product) -> Dict[str, Any]:
        ...
        """
        self.logger.info(f"Simulating Pet Store API data extraction for product: {product.name}")
        ...
        multiplier = category_multipliers.get(product.category, {"sales": 1.0, "revenue": 1.0, "inventory": 1.0})
    ...
    def _validate_api_response(self, response_data: Dict[str, Any]) -> bool:
        ...
        if not isinstance(response_data["sales_volume"], int) or response_data["sales_volume"] < 0:
            self.logger.warning("Invalid sales_volume in API response")
            return False
            
        if not isinstance(response_data["revenue"], (int, float)) or response_data["revenue"] < 0:
            self.logger.warning("Invalid revenue in API response")
            return False
            
        if not isinstance(response_data["inventory_level"], int) or response_data["inventory_level"] < 0:
