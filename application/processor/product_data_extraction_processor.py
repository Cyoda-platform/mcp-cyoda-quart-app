...
class ProductDataExtractionProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        try:
            ...
            if hasattr(entity, 'add_error'):
                entity.add_error(str(e), {"processor": "ProductDataExtractionProcessor"})
            
            raise

    async def _extract_pet_store_data(self, extraction_entity: DataExtraction) -> Dict[str, Any]:
        """
    ...
    async def _fetch_products_from_api(self, extraction_entity: DataExtraction) -> List[Dict[str, Any]]:
        """
                ...
                try:
                    ...
                    async with session.get(endpoint, timeout=30) as response:
                        ...
                        if response.status == 200:
                            ...
                            self.logger.info(f"Successfully fetched {len(data) if isinstance(data, list) else 0} items from {endpoint}")
                        else:
                            self.logger.warning(f"API call failed with status {response.status}: {endpoint}")
                    ...
                    self.logger.warning(f"Error calling API endpoint {endpoint}: {str(e)}")
                    extraction_entity.add_api_call(endpoint, 500)  # Internal Server Error
    ...
    def _transform_api_data(self, api_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            product_data["inventory_level"] = None  # Pet Store API doesn't provide inventory
            
            # Update extraction timestamp
            product_data["extracted_at"] = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    ...
    def _update_extraction_results(self, extraction_entity: DataExtraction, results: Dict[str, Any]) -> None:
        """
        ...
        total_records = extraction_entity.records_extracted + extraction_entity.records_failed
        if total_records > 0:
