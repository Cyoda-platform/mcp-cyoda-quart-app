    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["preparing"]:
                raise ProcessorError(self.name, f"Order must be preparing for shipping, current state: {entity.state}")
            
            # Get tracking information
            tracking_info = kwargs.get("trackingInfo") or kwargs.get("tracking_info", {})
            tracking_number = tracking_info.get("tracking_number", f"TRACK_{entity.entity_id[:8]}")
            ...
            logger.info(f"Successfully shipped order {entity.entity_id}, tracking: {tracking_number}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to process order shipping for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process order shipping: {str(e)}", e)
