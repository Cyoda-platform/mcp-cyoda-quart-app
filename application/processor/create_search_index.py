...
class CreateSearchIndex(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        try:
            ...
            self.logger.info(f"Search index created for HNItem {hn_item.technical_id}")
    ...
    def _create_hierarchy_index(self, hn_item: HNItem) -> None:
        ...
        try:
            ...
            if hn_item.parent:
                self.logger.debug(
                    f"Would index parent relationship: {hn_item.id} -> {hn_item.parent}"
                )
            ...
            if hn_item.kids:
                for kid_id in hn_item.kids:
                    self.logger.debug(
                        f"Would index child relationship: {hn_item.id} -> {kid_id}"
                    )
            ...
            if hn_item.poll:
                self.logger.debug(
                    f"Would index poll relationship: {hn_item.id} -> poll {hn_item.poll}"
                )

            if hn_item.parts:
                for part_id in hn_item.parts:
                    self.logger.debug(
                        f"Would index poll part: {hn_item.id} -> part {part_id}"
                    )
