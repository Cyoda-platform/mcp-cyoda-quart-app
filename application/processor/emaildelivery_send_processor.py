...
class EmailDeliverySendProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "EmailDeliverySendProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Sends email to subscriber"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            success = await self._send_email(subscriber_data.get("email"), email_content)
            ...
            if success:
                ...
                logger.info(f"Sent email to {subscriber_data.get('email')} for cat fact {entity.catFactId}")
            else:
    ...
    def _compose_email(self, subscriber_data: Dict[str, Any], catfact_data: Dict[str, Any]) -> Dict[str, str]:
        """Compose email content."""
        ...
        return {
            "subject": subject,
            "body": body,
            "tracking_pixel": f"[tracking pixel for delivery {self.entity_id}]"
        }
