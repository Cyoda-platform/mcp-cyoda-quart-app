...
class OrderCreationProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            logger.info(f"Successfully created order for pet {entity.petId} and owner {entity.ownerId}")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to process order creation for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process order creation: {str(e)}", e)
