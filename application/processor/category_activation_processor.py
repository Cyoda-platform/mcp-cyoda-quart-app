...
class CategoryActivationProcessor(CyodaProcessor):
    ...
    def __init__(self):
        super().__init__(
            ...
            description="Reactivates category"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state not in ["inactive"]:
                raise ProcessorError(self.name, f"Category must be inactive for activation, current state: {entity.state}")
            ...
            logger.exception(f"Failed to process category activation for entity {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to process category activation: {str(e)}", e)
