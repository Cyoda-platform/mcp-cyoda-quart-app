...
class EmailDeliveryRetryProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "EmailDeliveryRetryProcessor", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Retries failed email delivery"
        )
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            if entity.state != "failed":
                raise ProcessorError(self.name, f"EmailDelivery must be in failed state to retry, current state: {entity.state}")
            
            # Validate retry attempts are within limit
            max_retry_attempts = 3
            if entity.deliveryAttempts >= max_retry_attempts:
                raise ProcessorError(self.name, f"Maximum retry attempts ({max_retry_attempts}) exceeded")
            ...
            # Add retry metadata
            entity.add_metadata("retry_attempted_at", entity.lastAttemptDate.isoformat())
            entity.add_metadata("retry_count", entity.deliveryAttempts)
            
            logger.info(f"Retrying email delivery for subscriber {entity.subscriberId} (attempt {entity.deliveryAttempts + 1})")
            return entity
            
        except Exception as e:
            logger.exception(f"Failed to retry email delivery {entity.entity_id}")
            raise ProcessorError(self.name, f"Failed to retry email delivery: {str(e)}", e)
