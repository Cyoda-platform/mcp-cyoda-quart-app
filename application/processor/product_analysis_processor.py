...
class ProductAnalysisProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        try:
            ...
            product.set_performance_data(performance_score, trend_analysis, needs_restock)
    ...
    def _generate_trend_analysis(self, product: Product) -> Dict[str, Any]:
        ...
        sales_trend = "high" if sales_volume > 50 else "medium" if sales_volume > 20 else "low"
        revenue_trend = "high" if revenue > 500 else "medium" if revenue > 200 else "low"
        
        # Calculate inventory turnover estimate
        if inventory > 0 and sales_volume > 0:
            turnover_ratio = sales_volume / inventory
            turnover_category = "high" if turnover_ratio > 2 else "medium" if turnover_ratio > 1 else "low"
        else:
            turnover_ratio = 0.0
        ...
        return {
            ...
            "category_performance": self._analyze_category_performance(product.category),
    ...
    def _analyze_category_performance(self, category: str) -> Dict[str, Any]:
        ...
        benchmark = category_benchmarks.get(category, {"avg_sales": 30, "avg_revenue": 250})
    ...
    def _generate_recommendations(self, product: Product) -> list[str]:
        ...
        if product.category in ["dog", "cat"] and sales_volume < 30:
            recommendations.append("Popular category underperforming - investigate market factors")
