    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"Report {report.technical_id} emailed successfully to {report.email_recipient}")
            else:
                report.mark_email_failed()
                self.logger.error(f"Failed to email report {report.technical_id} to {report.email_recipient}")
...
For questions or support, please contact the development team.
    ...
    async def _send_email(
        ...
        try:
            ...
            self.logger.info("EMAIL SIMULATION - Report would be sent to: victoria.sagdieva@cyoda.com")
            self.logger.info(f"EMAIL CONTENT PREVIEW:\n{content.get('text_content', '')[:500]}...")
