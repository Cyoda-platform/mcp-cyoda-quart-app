...
class WeeklyScheduleFactRetrievalProcessor(CyodaProcessor):
    ...
    def __init__(
        self, name: str = "WeeklyScheduleFactRetrievalProcessor", description: str = ""
    ):
        super().__init__(
            name=name,
    ...
    async def process(self, entity: CyodaEntity, **kwargs) -> CyodaEntity:
        ...
        try:
            ...
            logger.info(
                f"Retrieved cat fact {catfact_id} for weekly schedule {entity.id}"
            )
            return entity

        except Exception as e:
            logger.exception(
                f"Failed to retrieve cat fact for weekly schedule {entity.entity_id}"
            )
            raise ProcessorError(
                self.name,
                f"Failed to retrieve cat fact for weekly schedule: {str(e)}",
                e,
            )
