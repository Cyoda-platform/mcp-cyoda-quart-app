...
class DataExtractionQueryParams(BaseModel):
    ...
    extraction_type: Optional[str] = Field(
        None, alias="extractionType", description="Filter by extraction type"
    )
    extraction_status: Optional[str] = Field(
        None, alias="extractionStatus", description="Filter by extraction status"
    )
    api_source: Optional[str] = Field(
        None, alias="apiSource", description="Filter by API source"
    )
    state: Optional[str] = Field(None, description="Filter by workflow state")
    offset: int = Field(0, ge=0, description="Pagination offset")
    limit: int = Field(50, ge=1, le=1000, description="Pagination limit")
...
class TransitionRequest(BaseModel):
    ...
    transition_name: str = Field(
        ..., alias="transitionName", description="Name of transition to execute"
    )
