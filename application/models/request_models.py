...
class BookingQueryParams(BaseModel):
    ...
    firstname: Optional[str] = Field(
        default=None, description="Filter by guest first name"
    )
    lastname: Optional[str] = Field(
        default=None, description="Filter by guest last name"
    )
    depositpaid: Optional[bool] = Field(
        default=None, description="Filter by deposit payment status"
    )
    min_price: Optional[int] = Field(
        default=None, alias="minPrice", description="Minimum total price filter"
    )
    max_price: Optional[int] = Field(
        default=None, alias="maxPrice", description="Maximum total price filter"
    )
...
class ReportQueryParams(BaseModel):
    ...
    display_format: Optional[str] = Field(
        default=None, alias="displayFormat", description="Filter by display format"
    )
    generated_by: Optional[str] = Field(
        default=None, alias="generatedBy", description="Filter by generator"
    )
    state: Optional[str] = Field(default=None, description="Filter by workflow state")
    limit: int = Field(default=50, description="Maximum number of results to return")
    offset: int = Field(default=0, description="Number of results to skip")
...
class SearchRequest(BaseModel):
    ...
    depositpaid: Optional[bool] = Field(
        default=None, description="Search by deposit status"
    )
...
class BookingFilterRequest(BaseModel):
    ...
    min_price: Optional[int] = Field(
        default=None, alias="minPrice", description="Minimum total price"
    )
    max_price: Optional[int] = Field(
        default=None, alias="maxPrice", description="Maximum total price"
    )
    start_date: Optional[str] = Field(
        default=None,
        alias="startDate",
        description="Start date for check-in filter (YYYY-MM-DD)",
    )
    end_date: Optional[str] = Field(
        default=None,
        alias="endDate",
        description="End date for check-in filter (YYYY-MM-DD)",
    )
    firstname: Optional[str] = Field(
        default=None, description="Filter by guest first name"
    )
...
class ReportGenerationRequest(BaseModel):
    ...
    report_type: str = Field(
        ..., alias="reportType", description="Type of report to generate"
    )
    display_format: str = Field(
        default="table",
        alias="displayFormat",
        description="Display format for the report",
    )
...
class TransitionRequest(BaseModel):
    ...
    parameters: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional parameters for the transition"
    )
...
class BulkOperationRequest(BaseModel):
    ...
    entity_ids: list[str] = Field(
        ..., alias="entityIds", description="List of entity IDs to operate on"
    )
