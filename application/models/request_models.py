...
class TransitionRequest(BaseModel):
    ...
    parameters: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional transition parameters")
...
class PetQueryParams(BaseModel):
    ...
    min_price: Optional[float] = Field(None, alias="minPrice", description="Minimum price filter")
    max_price: Optional[float] = Field(None, alias="maxPrice", description="Maximum price filter")
...
class UserQueryParams(BaseModel):
    ...
    emailVerified: Optional[bool] = Field(None, alias="emailVerified", description="Filter by email verification status")
    limit: int = Field(10, description="Number of results to return", ge=1, le=100)
    offset: int = Field(0, description="Number of results to skip", ge=0)
