...
class DataExtraction(CyodaEntity):
    ...
    status: str = Field(default="pending", description="Current extraction status")
    ...
    api_status_code: Optional[int] = Field(
        default=None, alias="apiStatusCode", description="Last API response status code"
    )
    ...
    timeout_seconds: int = Field(
        default=300, alias="timeoutSeconds", description="Extraction timeout in seconds"
    )
    ...
    def complete_extraction(
        self, records_extracted: int, records_processed: int
    ) -> None:
