...
class DataExtraction(CyodaEntity):
    ...
    api_endpoint: str = Field(
        ..., alias="apiEndpoint", description="API endpoint URL for data extraction"
    )
    ...
    frequency: Optional[str] = Field(
        default="weekly", description="Extraction frequency (daily, weekly, monthly)"
    )
    ...
    # Validation rules
    ALLOWED_EXTRACTION_TYPES: ClassVar[List[str]] = ["scheduled", "manual", "on_demand"]
    ...
    ALLOWED_FORMATS: ClassVar[List[str]] = ["json", "xml", "csv"]
    ...
    def validate_extraction_status(cls, v: str) -> str:
        ...
        if v not in cls.ALLOWED_STATUSES:
            raise ValueError(
                f"Extraction status must be one of: {cls.ALLOWED_STATUSES}"
            )
        return v
    ...
    def validate_api_response_format(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None and v not in cls.ALLOWED_FORMATS:
            raise ValueError(
                f"API response format must be one of: {cls.ALLOWED_FORMATS}"
            )
        return v
    ...
    def complete_extraction(
        self, records_extracted: int, records_processed: int, records_failed: int = 0
    ) -> None:
