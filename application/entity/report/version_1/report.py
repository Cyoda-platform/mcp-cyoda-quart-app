...
class Report(CyodaEntity):
    ...
    report_period_start: str = Field(..., description="Start date for the reporting period (ISO 8601 format)")
    report_period_end: str = Field(..., description="End date for the reporting period (ISO 8601 format)")
    
    # Optional fields
    summary_data: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Aggregated analysis results and metrics"
    )
    generated_at: Optional[str] = Field(
        default=None,
        description="When the report was generated (ISO 8601 format)"
    )
    ...
    status: Optional[str] = Field(
        default=None,
        description="Report status (generating, ready, sent, failed)"
    )
    generation_started_at: Optional[str] = Field(
        default=None,
        description="When report generation started (ISO 8601 format)"
    )
    total_comments: Optional[int] = Field(
        default=None,
        description="Total number of comments in the report period"
    )
    ...
    top_keywords: Optional[list] = Field(
        default=None,
        description="Top keywords from the analysis period"
    )
    toxicity_summary: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Summary of toxicity metrics"
    )
    ...
    def validate_business_logic(self) -> "Report":
        ...
        try:
            start_dt = datetime.fromisoformat(self.report_period_start.replace('Z', '+00:00'))
            end_dt = datetime.fromisoformat(self.report_period_end.replace('Z', '+00:00'))
    ...
    def set_generation_started_at(self) -> None:
        ...
        self.generation_started_at = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")

    def set_report_metrics(self, total_comments: int, avg_sentiment: float, 
                          top_keywords: list, toxicity_summary: Dict[str, Any]) -> None:
