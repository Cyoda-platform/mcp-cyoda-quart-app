...
class Product(CyodaEntity):
    ...
    pet_id: Optional[int] = Field(
        default=None, alias="petId", description="Pet ID from the Pet Store API"
    )
    name: str = Field(..., description="Name of the pet/product")
    category_id: Optional[int] = Field(
        default=None, alias="categoryId", description="Category ID from Pet Store API"
    )
    ...
    price: Optional[float] = Field(
        default=None, description="Product price for performance analysis"
    )
    sales_volume: Optional[int] = Field(
        default=0, alias="salesVolume", description="Number of units sold"
    )
    revenue: Optional[float] = Field(
        default=0.0, description="Total revenue generated by this product"
    )
    inventory_count: Optional[int] = Field(
        default=0, alias="inventoryCount", description="Current inventory count"
    )
    ...
    def validate_business_logic(self) -> "Product":
        ...
        if self.sales_volume and self.sales_volume > 0:
            if not self.revenue or self.revenue <= 0:
                raise ValueError(
                    "Revenue must be positive when sales volume is positive"
                )
    ...
    def calculate_performance_score(self) -> float:
        ...
        base_score = min(
            50.0, (self.sales_volume / 10) * 10
        )  # Up to 50 points for volume
        revenue_score = min(
            30.0, (self.revenue / 1000) * 10
        )  # Up to 30 points for revenue
        inventory_score = (
            20.0 if self.inventory_count and self.inventory_count > 0 else 0.0
        )
