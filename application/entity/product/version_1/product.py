...
class Product(CyodaEntity):
    ...
    def validate_business_logic(self) -> "Product":
        ...
        if self.inventory_level is not None and self.stock_status:
            if self.inventory_level == 0 and self.stock_status not in [
                "OUT_OF_STOCK",
                "UNKNOWN",
            ]:
                raise ValueError(
                    "Products with zero inventory should have OUT_OF_STOCK status"
                )
            elif self.inventory_level > 0 and self.stock_status == "OUT_OF_STOCK":
                raise ValueError(
                    "Products with inventory cannot have OUT_OF_STOCK status"
                )
    ...
    def set_data_extracted(self) -> None:
        ...
        self.data_extracted_at = (
            datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
        )
        self.update_timestamp()

    def set_analyzed(
        self, performance_score: float, trend_analysis: Dict[str, Any]
    ) -> None:
