...
class CatFact(CyodaEntity):
    ...
    id: Optional[int] = Field(
        default=None, description="Unique identifier for the cat fact"
    )
    ...
    retrievedDate: Optional[datetime] = Field(
        default=None, description="Date and time when the fact was retrieved from API"
    )
    scheduledSendDate: Optional[datetime] = Field(
        default=None, description="Date and time when this fact is scheduled to be sent"
    )
    apiFactId: Optional[str] = Field(
        default=None, description="Original ID from the Cat Fact API (if available)"
    )
