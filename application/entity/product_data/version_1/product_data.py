...
class ProductData(CyodaEntity):
    ...
    product_id: str = Field(
        ..., description="Unique product identifier from Pet Store API"
    )
    ...
    sales_volume: Optional[int] = Field(
        default=0, alias="salesVolume", description="Total sales volume for the product"
    )
    revenue: Optional[float] = Field(
        default=0.0, description="Total revenue generated by the product"
    )
    stock_level: Optional[int] = Field(
        default=0, alias="stockLevel", description="Current stock level"
    )
    ...
    def calculate_performance_metrics(self) -> None:
        ...
        self.is_slow_moving = (self.sales_volume or 0) < 5 and (
            self.stock_level or 0
        ) > 50
