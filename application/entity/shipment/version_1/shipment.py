...
class Shipment(CyodaEntity):
    ...
    orderId: str = Field(
        ..., alias="orderId", description="Associated order identifier"
    )
    status: str = Field(
        ..., description="Shipment status: PICKING, WAITING_TO_SEND, SENT, DELIVERED"
    )
    ...
    shippedAt: Optional[str] = Field(
        default=None, alias="shippedAt", description="Timestamp when shipment was sent"
    )
    ...
    def validate_business_logic(self) -> "Shipment":
        ...
        for line in self.lines:
            ...
            if qty_picked > qty_ordered:
                raise ValueError(
                    f"Picked quantity ({qty_picked}) cannot exceed ordered quantity ({qty_ordered}) for SKU {line.get('sku')}"
                )

            if qty_shipped > qty_picked:
                raise ValueError(
                    f"Shipped quantity ({qty_shipped}) cannot exceed picked quantity ({qty_picked}) for SKU {line.get('sku')}"
                )
    ...
    def add_line_item(self, sku: str, qty_ordered: int) -> None:
        ...
        self.lines.append(
            {"sku": sku, "qtyOrdered": qty_ordered, "qtyPicked": 0, "qtyShipped": 0}
        )
        ...
        for line in self.lines:
            if line.get("sku") == sku:
                if qty_picked > line.get("qtyOrdered", 0):
                    raise ValueError(
                        f"Picked quantity ({qty_picked}) cannot exceed ordered quantity ({line.get('qtyOrdered', 0)})"
                    )
        ...
        for line in self.lines:
            if line.get("sku") == sku:
                if qty_shipped > line.get("qtyPicked", 0):
                    raise ValueError(
                        f"Shipped quantity ({qty_shipped}) cannot exceed picked quantity ({line.get('qtyPicked', 0)})"
                    )
    ...
    def mark_shipped(
        self, tracking_number: Optional[str] = None, carrier: Optional[str] = None
    ) -> None:
        """Mark shipment as shipped"""
        if self.status != "WAITING_TO_SEND":
            raise ValueError(
                f"Cannot mark shipment as shipped from status: {self.status}"
            )
    ...
    def mark_delivered(self) -> None:
        ...
        if self.status != "SENT":
            raise ValueError(
                f"Cannot mark shipment as delivered from status: {self.status}"
            )
    ...
    def create_from_order(
        cls, shipment_id: str, order_data: Dict[str, Any]
    ) -> "Shipment":
