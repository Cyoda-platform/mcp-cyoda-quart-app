...
class Store(CyodaEntity):
    ...
    total_pets: Optional[int] = Field(
        None,
        alias="totalPets",
        description="Total number of pets in store"
    )
    available_pets: Optional[int] = Field(
        None,
        alias="availablePets",
        description="Number of available pets"
    )
    pending_pets: Optional[int] = Field(
        None,
        alias="pendingPets",
        description="Number of pending pets"
    )
    sold_pets: Optional[int] = Field(
        None,
        alias="soldPets",
        description="Number of sold pets"
    )
    ...
    def update_inventory(self, inventory_data: Dict[str, int]) -> None:
        ...
        self.last_inventory_update = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    ...
    def is_inventory_current(self, max_age_hours: int = 24) -> bool:
        ...
        try:
            last_update = datetime.fromisoformat(self.last_inventory_update.replace("Z", "+00:00"))
    ...
    def is_analyzed(self) -> bool:
        ...
        return self.performance_metrics is not None and self.last_analyzed_at is not None
