...
class Pet(CyodaEntity):
    ...
    ALLOWED_VACCINATION_STATUS: ClassVar[List[str]] = [
        "Up to Date", "Needs Update", "Not Vaccinated", "Partial"
    ]
    ...
    def validate_health_status(cls, v: str) -> str:
        ...
        if v not in cls.ALLOWED_HEALTH_STATUS:
            raise ValueError(f"Health status must be one of: {cls.ALLOWED_HEALTH_STATUS}")
        return v
    ...
    def validate_vaccination_status(cls, v: str) -> str:
        ...
        if v not in cls.ALLOWED_VACCINATION_STATUS:
            raise ValueError(f"Vaccination status must be one of: {cls.ALLOWED_VACCINATION_STATUS}")
        return v
    ...
    def validate_adoption_status(cls, v: str) -> str:
        ...
        if v not in cls.ALLOWED_ADOPTION_STATUS:
            raise ValueError(f"Adoption status must be one of: {cls.ALLOWED_ADOPTION_STATUS}")
        return v
    ...
    def validate_business_logic(self) -> "Pet":
        ...
        if self.health_status == "Under Treatment" and self.adoption_status == "Available":
    ...
    def is_available_for_adoption(self) -> bool:
        ...
        return (
            ...
            and self.health_status in ["Healthy", "Recovering"]
        )
