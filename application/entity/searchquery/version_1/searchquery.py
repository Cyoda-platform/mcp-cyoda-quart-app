...
class SearchQuery(CyodaEntity):
    ...
    query_text: str = Field(
        ...,
        alias="queryText",
        description="Search terms and keywords"
    )
    ...
    cached_at: Optional[str] = Field(
        default=None,
        alias="cachedAt",
        description="When results were cached"
    )
    cache_ttl: int = Field(
    ...
    cache_key: Optional[str] = Field(
        default=None,
        alias="cacheKey",
        description="Generated cache key for the query"
    )
    ...
    def validate_query_text(cls, v: str) -> str:
        ...
        if len(v) > cls.MAX_QUERY_LENGTH:
            raise ValueError(f"Query text must be at most {cls.MAX_QUERY_LENGTH} characters")
        return v.strip()
