...
class MedicalRecord(CyodaEntity):
    ...
    appointment_id: Optional[str] = Field(
        None, description="Reference to related appointment"
    )
    ...
    def validate_visit_date(cls, v):
        ...
        try:
            ...
            raise ValueError(
                "Invalid visit date format. Use ISO format (YYYY-MM-DDTHH:MM:SSZ)"
            )
    ...
    def validate_follow_up_date(cls, v):
        ...
        if v is not None:
            ...
            try:
                ...
                raise ValueError(
                    "Invalid follow-up date format. Use ISO format (YYYY-MM-DDTHH:MM:SSZ)"
                )
    ...
    def is_follow_up_overdue(self) -> bool:
        ...
        try:
            follow_up_dt = datetime.fromisoformat(
                self.follow_up_date.replace("Z", "+00:00")
            )
