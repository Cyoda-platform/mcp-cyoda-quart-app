...
class User(CyodaEntity):
    ...
    password: Optional[str] = Field(
        default=None,
        description="Encrypted password (not exposed in API responses)"
    )
    ...
    def validate_preferences(cls, v: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        allowed_keys = {
            "petTypes", "notifications", "newsletter", "language", "timezone"
        }
    ...
    def validate_business_logic(self) -> "User":
        ...
        if self.state in ["registered", "active", "suspended"] and not self.registeredAt:
            # This will be set by the processor, so we don't enforce it here
            pass
