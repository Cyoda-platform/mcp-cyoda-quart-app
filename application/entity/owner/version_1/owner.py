...
class Owner(CyodaEntity):
    ...
    emergency_contact_name: Optional[str] = Field(
        None, description="Emergency contact person"
    )
    emergency_contact_phone: Optional[str] = Field(
        None, description="Emergency contact phone"
    )
    preferred_vet_id: Optional[str] = Field(
        None, description="Reference to preferred veterinarian"
    )
    communication_preferences: Optional[str] = Field(
        None, description="Email/SMS/Phone preferences"
    )
    ...
    def validate_communication_preferences(cls, v):
        ...
        if v is not None:
            ...
            if v.lower() not in allowed_prefs:
                raise ValueError(
                    f"Communication preferences must be one of: {', '.join(allowed_prefs)}"
                )
            return v.lower()
        return v
