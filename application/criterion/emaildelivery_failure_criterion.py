...
class EmailDeliveryFailureCriterion(CyodaCriteriaChecker):
    ...
    def __init__(self, name: str = "EmailDeliveryFailureCriterion", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Checks if email delivery failed"
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to check failure criteria for email delivery {entity.entity_id}")
            raise CriteriaError(self.name, f"Failed to check failure criteria: {str(e)}", e)
