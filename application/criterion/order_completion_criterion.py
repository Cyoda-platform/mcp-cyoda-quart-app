...
class OrderCompletionCriterion(CyodaCriteriaChecker):
    ...
    async def check(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        ...
        """
        try:
            self.logger.info(f"Checking order completion for entity {getattr(entity, 'technical_id', '<unknown>')}")
            ...
            if not order_id:
                self.logger.warning(f"No order_id provided in order_context for pet {pet.technical_id}")
                return False
            ...
            if order_completed:
                self.logger.info(f"Order {order_id} is completed for pet {pet.technical_id}")
            else:
                self.logger.info(f"Order {order_id} is not completed for pet {pet.technical_id}")
