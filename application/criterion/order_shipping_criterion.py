    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state != "preparing":
                logger.debug(
                    f"Order {entity.entity_id} is not preparing, current state: {entity.state}"
                )
                return False
            ...
            documentation_complete = entity.get_metadata(
            ...
            if len(address_parts) < 2:
                logger.debug(
                    f"Order {entity.entity_id} has invalid delivery address format"
                )
                return False

            # Check if shipping method is available (from kwargs or metadata)
            shipping_method = kwargs.get("shipping_method") or entity.get_metadata(
            ...
            if shipping_method not in available_methods:
                logger.debug(
                    f"Order {entity.entity_id} has invalid shipping method: {shipping_method}"
                )
                return False
            ...
            logger.exception(
                f"Failed to check shipping criteria for order {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check order shipping: {str(e)}", e
            )
