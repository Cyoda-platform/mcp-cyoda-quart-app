...
class PetValidationCriterion(CyodaCriteriaChecker):
    ...
    def __init__(self, name: str = "PetValidationCriterion", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Validate that a pet has all required information for activation"
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to check pet validation criteria for entity {entity.entity_id}")
            raise CriteriaError(self.name, f"Failed to check pet validation criteria: {str(e)}", e)
    ...
    async def _validate_microchip_unique(self, microchip_id: Optional[str], pet_id: str) -> bool:
