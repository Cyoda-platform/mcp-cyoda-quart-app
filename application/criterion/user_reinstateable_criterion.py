...
class UserReinstateableCriterion(CyodaCriteriaChecker):
    ...
    async def check(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        ...
        try:
            ...
            if any(reason in suspension_reason.lower() for reason in non_reinstateable_reasons):
                self.logger.warning(
            ...
            if suspension_date_str:
                try:
                    suspension_date = datetime.fromisoformat(suspension_date_str.replace("Z", "+00:00"))
                    ...
                    self.logger.warning(f"Invalid suspension date format for User {user.technical_id}")
                    return False
