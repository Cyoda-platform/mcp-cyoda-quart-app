...
class EmailDeliveryBounceCriterion(CyodaCriteriaChecker):
    ...
    def __init__(
        self, name: str = "EmailDeliveryBounceCriterion", description: str = ""
    ):
        super().__init__(
            name=name,
            description=description
            or "Checks if email delivery resulted in a permanent bounce",
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            logger.exception(
                f"Failed to check bounce criteria for email delivery {entity.entity_id}"
            )
            raise CriteriaError(
    ...
    def _check_bounce_classification_with_provider(self, entity: EmailDelivery) -> str:
        ...
        if entity.errorMessage:
            ...
            if any(
                ...
                for indicator in ["hard", "permanent", "invalid"]
            ):
                return "hard"
            else:
                return "soft"
