...
class SubscriberBounceCriterion(CyodaCriteriaChecker):
    ...
    def __init__(self, name: str = "SubscriberBounceCriterion", description: str = ""):
        super().__init__(
            ...
            or "Checks if subscriber should be marked as bounced based on email delivery failures",
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            logger.exception(
                f"Failed to check bounce criteria for subscriber {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check bounce criteria: {str(e)}", e
            )
    ...
    def _is_permanent_bounce(self, error_message: str) -> bool:
        ...
        return any(
            indicator in error_message.lower() for indicator in permanent_indicators
        )
    ...
    def _is_hard_bounce(self, error_message: str) -> bool:
        ...
        return any(
            indicator in error_message.lower() for indicator in hard_bounce_indicators
        )
