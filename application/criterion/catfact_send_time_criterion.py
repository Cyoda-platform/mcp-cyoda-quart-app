...
class CatFactSendTimeCriterion(CyodaCriteriaChecker):
    ...
    def __init__(self, name: str = "CatFactSendTimeCriterion", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Checks if it's time to send the scheduled cat fact"
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to check send time criteria for cat fact {entity.entity_id}")
            raise CriteriaError(self.name, f"Failed to check send time criteria: {str(e)}", e)
