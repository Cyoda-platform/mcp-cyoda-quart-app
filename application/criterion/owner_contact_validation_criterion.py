...
class OwnerContactValidationCriterion(CyodaCriteriaChecker):
    ...
    def __init__(
        self, name: str = "OwnerContactValidationCriterion", description: str = ""
    ):
        super().__init__(
            name=name,
            description=description
            or "Validate that owner's contact information is correct and verified",
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if not self._validate_email_format(owner.email):
                logger.info(
                    f"Email format validation failed for owner {owner.owner_id}"
                )
                return False
            ...
            if not await self._check_email_deliverable(owner.email):
                logger.info(
                    f"Email deliverability check failed for owner {owner.owner_id}"
                )
                return False
            ...
            if not self._validate_phone_format(owner.phone):
                logger.info(
                    f"Phone format validation failed for owner {owner.owner_id}"
                )
                return False
            ...
            if not await self._check_contact_verification(owner):
                logger.info(
                    f"Contact verification check failed for owner {owner.owner_id}"
                )
                return False
            ...
            if not self._validate_emergency_contact(owner):
                logger.info(
                    f"Emergency contact validation failed for owner {owner.owner_id}"
                )
                return False
            ...
            logger.exception(
                f"Failed to check owner contact validation criteria for entity {entity.entity_id}"
            )
            raise CriteriaError(
                self.name,
                f"Failed to check owner contact validation criteria: {str(e)}",
                e,
            )
