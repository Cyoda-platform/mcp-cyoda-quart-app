...
class WeeklyScheduleTimeCriterion(CyodaCriteriaChecker):
    ...
    def __init__(self, name: str = "WeeklyScheduleTimeCriterion", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Checks if it's time to retrieve cat fact for weekly distribution"
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to check time criteria for weekly schedule {entity.entity_id}")
            raise CriteriaError(self.name, f"Failed to check time criteria: {str(e)}", e)
