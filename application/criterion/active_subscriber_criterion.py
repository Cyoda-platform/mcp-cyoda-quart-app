...
class ActiveSubscriberCriterion(CyodaCriteriaChecker):
    ...
    def __init__(self, name: str = "ActiveSubscriberCriterion", description: str = ""):
        super().__init__(
            ...
            or "Checks if subscriber is active and can receive emails",
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            logger.exception(
                f"Failed to check active criteria for subscriber {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check active criteria: {str(e)}", e
            )
