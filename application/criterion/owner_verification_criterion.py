    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state != "pending_verification":
                logger.debug(
                    f"Owner {entity.entity_id} is not pending verification, current state: {entity.state}"
                )
                return False
            ...
            if not verification_token:
                logger.debug(
                    f"No verification token provided for owner {entity.entity_id}"
                )
                return False
            ...
            if not stored_token:
                logger.debug(
                    f"No stored verification token found for owner {entity.entity_id}"
                )
                return False

            if stored_token != verification_token:
                logger.debug(
                    f"Verification token mismatch for owner {entity.entity_id}"
                )
                return False
            ...
            logger.exception(
                f"Failed to check verification criteria for owner {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check owner verification: {str(e)}", e
            )
