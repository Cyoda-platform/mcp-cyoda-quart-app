    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state != "suspended":
                logger.debug(f"Owner {entity.entity_id} is not suspended, current state: {entity.state}")
                return False
            ...
            try:
                suspension_date = datetime.fromisoformat(suspended_at.replace('Z', '+00:00'))
                current_date = datetime.now(timezone.utc)
                
                # Default suspension period is 30 days
                suspension_period_days = kwargs.get("suspension_period_days", 30)
                suspension_end_date = suspension_date + timedelta(days=suspension_period_days)
                
                if current_date < suspension_end_date:
                    logger.debug(f"Suspension period not complete for owner {entity.entity_id}")
                    return False
                    
            except (ValueError, TypeError) as e:
                logger.warning(f"Invalid suspension date format for owner {entity.entity_id}: {e}")
                return False
            ...
            if not conditions_met:
                logger.debug(f"Reinstatement conditions not met for owner {entity.entity_id}")
                return False
            
            # Check for new violations during suspension
            new_violations = entity.get_metadata("violations_during_suspension", [])
            if new_violations:
                logger.debug(f"Owner {entity.entity_id} has new violations during suspension: {new_violations}")
                return False
            ...
            logger.exception(f"Failed to check reinstate criteria for owner {entity.entity_id}")
            raise CriteriaError(self.name, f"Failed to check owner reinstatement: {str(e)}", e)
