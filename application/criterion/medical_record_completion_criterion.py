...
class MedicalRecordCompletionCriterion(CyodaCriteriaChecker):
    ...
    def __init__(self, name: str = "MedicalRecordCompletionCriterion", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Validate that a medical record is ready for completion"
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if not self._has_required_fields(record):
                logger.info(f"Required fields validation failed for record {record.record_id}")
                return False
            
            # Check if diagnosis or treatment is provided
            if not self._has_medical_content(record):
                logger.info(f"Medical content validation failed for record {record.record_id}")
                return False
            
            # Check if visit date is valid
            if not self._is_visit_date_valid(record.visit_date):
                logger.info(f"Visit date validation failed for record {record.record_id}")
                return False
            
            logger.info(f"All completion validations passed for record {record.record_id}")
            return True
            
        except Exception as e:
            logger.exception(f"Failed to check medical record completion criteria for entity {entity.entity_id}")
            raise CriteriaError(self.name, f"Failed to check medical record completion criteria: {str(e)}", e)
    ...
    def _has_medical_content(self, record: MedicalRecord) -> bool:
        """Check if diagnosis or treatment is provided."""
        return bool(record.diagnosis and record.diagnosis.strip()) or bool(record.treatment and record.treatment.strip())
