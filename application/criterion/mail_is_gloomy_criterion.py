"""
MailIsGloomyCriterion for Cyoda Client Application

Determines if a mail entity should be processed as a gloomy mail based on the isHappy field.
"""

from typing import Any

from application.entity.mail import Mail
from common.entity.entity_casting import cast_entity
from common.processor.base import CyodaCriteriaChecker, CyodaEntity


class MailIsGloomyCriterion(CyodaCriteriaChecker):
    """
    Criterion that determines if a mail entity should be processed as a gloomy mail.
    """

    def __init__(self) -> None:
        super().__init__(
            name="MailIsGloomyCriterion",
            description="Determines if a mail entity should be processed as a gloomy mail based on the isHappy field",
        )

    async def check(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        """
        Check if the mail entity should be processed as a gloomy mail.

        Args:
            entity: The CyodaEntity to check (expected to be Mail)
            **kwargs: Additional criteria parameters

        Returns:
            True if the mail is marked as gloomy (not happy), False otherwise
        """
        try:
            self.logger.info(
                f"Checking if Mail entity {getattr(entity, 'technical_id', '<unknown>')} is gloomy"
            )

            # Cast the entity to Mail for type-safe operations
            mail_entity = cast_entity(entity, Mail)

            # Check if mail.isHappy equals false
            is_gloomy = mail_entity.is_happy is False

            self.logger.info(
                f"Mail entity {mail_entity.technical_id} isGloomy check result: {is_gloomy}"
            )

            return is_gloomy

        except Exception as e:
            self.logger.error(
                f"Error checking Mail entity {getattr(entity, 'technical_id', '<unknown>')}: {str(e)}"
            )
            return False
