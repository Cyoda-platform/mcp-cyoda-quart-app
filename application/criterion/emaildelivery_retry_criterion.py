...
class EmailDeliveryRetryCriterion(CyodaCriteriaChecker):
    ...
    def __init__(
        self, name: str = "EmailDeliveryRetryCriterion", description: str = ""
    ):
        super().__init__(
            name=name,
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            logger.exception(
                f"Failed to check retry criteria for email delivery {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check retry criteria: {str(e)}", e
            )
