    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state not in cancellable_states:
                logger.debug(
                    f"Order {entity.entity_id} cannot be cancelled, current state: {entity.state}"
                )
                return False
            ...
            if not cancellation_reason:
                logger.debug(
                    f"No cancellation reason provided for order {entity.entity_id}"
                )
                return False
            ...
            if cancellation_reason not in valid_reasons:
                logger.debug(
                    f"Invalid cancellation reason '{cancellation_reason}' for order {entity.entity_id}"
                )
                return False
            ...
            if cancellation_policy == "strict" and entity.state == "confirmed":
                ...
                logger.debug(
                    f"Order {entity.entity_id} cannot be cancelled due to strict policy"
                )
                return False
            ...
            if order_age_hours > max_cancellation_hours:
                logger.debug(
                    f"Order {entity.entity_id} is too old for cancellation: {order_age_hours} hours"
                )
                return False

            logger.debug(
                f"Order {entity.entity_id} can be cancelled for reason: {cancellation_reason}"
            )
            return True

        except Exception as e:
            logger.exception(
                f"Failed to check cancellation criteria for order {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check order cancellation: {str(e)}", e
            )
