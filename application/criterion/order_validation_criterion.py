    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state != "placed":
                logger.debug(f"Order {entity.entity_id} is not placed, current state: {entity.state}")
                return False
            
            # Check if total amount is positive
            if entity.totalAmount <= 0:
                logger.debug(f"Order {entity.entity_id} has invalid total amount: {entity.totalAmount}")
                return False
            ...
            if pet:
                if hasattr(pet, 'state') and pet.state not in ["available", "pending"]:
                    logger.debug(f"Pet {pet.entity_id} is not available, current state: {pet.state}")
                    return False
                
                if hasattr(pet, 'id') and entity.petId != pet.id:
                    logger.debug(f"Order pet ID {entity.petId} does not match provided pet {pet.id}")
                    return False
            ...
            if owner:
                ...
                if hasattr(owner, 'id') and entity.ownerId != owner.id:
                    logger.debug(f"Order owner ID {entity.ownerId} does not match provided owner {owner.id}")
                    return False
            ...
            logger.exception(f"Failed to check validation criteria for order {entity.entity_id}")
            raise CriteriaError(self.name, f"Failed to check order validation: {str(e)}", e)
