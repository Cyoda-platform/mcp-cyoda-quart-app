    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state != "active":
                logger.debug(f"Owner {entity.entity_id} is not active, current state: {entity.state}")
                return False
            ...
            if not suspension_reason:
                logger.debug(f"No suspension reason provided for owner {entity.entity_id}")
                return False
            ...
            if suspension_reason not in valid_reasons:
                logger.debug(f"Invalid suspension reason '{suspension_reason}' for owner {entity.entity_id}")
                return False
            ...
            if not justification:
                logger.debug(f"No justification provided for suspending owner {entity.entity_id}")
                return False
            
            logger.debug(f"Owner {entity.entity_id} can be suspended for reason: {suspension_reason}")
            return True
            
        except Exception as e:
            logger.exception(f"Failed to check suspension criteria for owner {entity.entity_id}")
            raise CriteriaError(self.name, f"Failed to check owner suspension: {str(e)}", e)
