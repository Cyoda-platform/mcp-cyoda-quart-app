    ...
    async def check(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        ...
        try:
            ...
            if not contract_signed:
                self.logger.info(
                    f"Pet {pet.technical_id} adoption contract not signed"
                )
                return False
            ...
            if not fee_received:
                self.logger.info(
                    f"Pet {pet.technical_id} adoption fee not received"
                )
                return False
            ...
            if not pet.vaccinated:
                self.logger.info(
                    f"Pet {pet.technical_id} is not vaccinated"
                )
                return False

            self.logger.info(
                f"Pet {pet.technical_id} is ready for adoption"
            )
            return True
