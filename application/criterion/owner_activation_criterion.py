    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state != "inactive":
                logger.debug(
                    f"Owner {entity.entity_id} is not inactive, current state: {entity.state}"
                )
                return False
            ...
            if not good_standing:
                logger.debug(
                    f"Owner {entity.entity_id} account is not in good standing"
                )
                return False
            ...
            if restrictions:
                logger.debug(
                    f"Owner {entity.entity_id} has active restrictions: {restrictions}"
                )
                return False
            ...
            logger.exception(
                f"Failed to check activation criteria for owner {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check owner activation: {str(e)}", e
            )
