    ...
    async def check(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        ...
        try:
            ...
            if not collection.processing_errors:
                self.logger.info(f"No processing errors found for collection {collection.technical_id}")
                return False

            # Analyze error severity and frequency
            critical_errors = self._identify_critical_errors(collection.processing_errors)
            
            if not critical_errors:
                self.logger.info(f"No critical errors found for collection {collection.technical_id}")
                return False
            ...
            if validation_errors and len(validation_errors) >= 3:  # Multiple validation issues
                self.logger.error(
