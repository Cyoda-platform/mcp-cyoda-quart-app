...
class WeatherStationValidationCriterion(CyodaCriteriaChecker):
    ...
    async def check(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        ...
        try:
            ...
            if not weather_station.station_id or len(weather_station.station_id.strip()) == 0:
                self.logger.warning(
                    f"Station {weather_station.technical_id} has invalid station_id: '{weather_station.station_id}'"
                )
                return False

            if not weather_station.station_name or len(weather_station.station_name.strip()) == 0:
                self.logger.warning(
            ...
            if not self._is_in_canada(weather_station.latitude, weather_station.longitude):
                self.logger.warning(
            ...
            if weather_station.first_year is not None and weather_station.last_year is not None:
                ...
                current_year = 2024  # Could use datetime.now().year but keeping it simple
                if weather_station.first_year < 1800 or weather_station.first_year > current_year:
                    self.logger.warning(
                        f"Station {weather_station.technical_id} has unrealistic first_year: {weather_station.first_year}"
                    )
                    return False

                if weather_station.last_year < 1800 or weather_station.last_year > current_year:
                    self.logger.warning(
    ...
    def _is_in_canada(self, latitude: float, longitude: float) -> bool:
        ...
        return (
            canada_bounds["min_lat"] <= latitude <= canada_bounds["max_lat"] and
            canada_bounds["min_lon"] <= longitude <= canada_bounds["max_lon"]
        )
