"""
CommentAnalysisRequestAnalysisFailureCriterion for Cyoda Client Application
...
class CommentAnalysisRequestAnalysisFailureCriterion(CyodaCriteriaChecker):
    """
    ...
    def __init__(self) -> None:
        super().__init__(
            name="CommentAnalysisRequestAnalysisFailureCriterion",
    ...
    async def check(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        ...
        Returns:
            True if analysis report is missing/incomplete or error message is set, False otherwise
        """
        try:
            self.logger.info(
                f"Checking analysis failure for entity {getattr(entity, 'technical_id', '<unknown>')}"
            )
            ...
            if report is None:
                self.logger.info(
                    f"Entity {request_entity.technical_id} has no analysis report - analysis failed"
                )
                return True
            ...
            if report.generated_at is None:
                self.logger.info(
                    f"Entity {request_entity.technical_id} has incomplete analysis report - analysis failed"
                )
                return True
