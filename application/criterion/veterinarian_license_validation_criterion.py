...
class VeterinarianLicenseValidationCriterion(CyodaCriteriaChecker):
    ...
    def __init__(self, name: str = "VeterinarianLicenseValidationCriterion", description: str = ""):
        super().__init__(
            name=name,
            description=description or "Validate veterinarian's license and credentials"
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to check veterinarian license validation criteria for entity {entity.entity_id}")
            raise CriteriaError(self.name, f"Failed to check veterinarian license validation criteria: {str(e)}", e)
    ...
    async def _check_name_matches_license(self, vet: Veterinarian) -> bool:
        ...
        logger.debug(f"Mock validation: Name {full_name} matches license {vet.license_number}")
        return True
    
    async def _validate_specialization_credentials(self, specialization: Optional[str]) -> bool:
        ...
        logger.debug(f"Mock validation: Specialization {specialization} credentials are valid")
        return True
