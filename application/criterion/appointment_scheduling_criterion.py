...
class AppointmentSchedulingCriterion(CyodaCriteriaChecker):
    ...
    def __init__(
        self, name: str = "AppointmentSchedulingCriterion", description: str = ""
    ):
        super().__init__(
            name=name,
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if not self._is_appointment_in_future(appointment.appointment_date):
                logger.info(
                    f"Appointment date validation failed for appointment {appointment.appointment_id}"
                )
                return False
            ...
            if not self._is_duration_reasonable(appointment.duration_minutes):
                logger.info(
                    f"Duration validation failed for appointment {appointment.appointment_id}"
                )
                return False
            ...
            if not self._is_appointment_type_valid(appointment.appointment_type):
                logger.info(
                    f"Appointment type validation failed for appointment {appointment.appointment_id}"
                )
                return False

            logger.info(
                f"All scheduling validations passed for appointment {appointment.appointment_id}"
            )
            return True

        except Exception as e:
            logger.exception(
                f"Failed to check appointment scheduling criteria for entity {entity.entity_id}"
            )
            raise CriteriaError(
                self.name,
                f"Failed to check appointment scheduling criteria: {str(e)}",
                e,
            )
