    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state != "pending":
                logger.debug(
                    f"Pet {entity.entity_id} is not pending, current state: {entity.state}"
                )
                return False
            ...
            if owner:
                ...
                if hasattr(owner, "id") and entity.ownerId != owner.id:
                    logger.debug(
                        f"Pet owner {entity.ownerId} does not match provided owner {owner.id}"
                    )
                    return False
            ...
            # If only owner_id is provided, check it matches
                ...
                logger.debug(
                    f"Pet owner {entity.ownerId} does not match provided owner_id {owner_id}"
                )
                return False
            ...
            logger.debug(
                f"Pet {entity.entity_id} can be sold to owner {entity.ownerId}"
            )
            return True

        except Exception as e:
            logger.exception(
                f"Failed to check sale validation criteria for pet {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check pet sale validation: {str(e)}", e
            )
