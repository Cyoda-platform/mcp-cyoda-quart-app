...
class OrderReturnCriterion(CyodaCriteriaChecker):
    ...
    def __init__(self):
        super().__init__(
            name="OrderReturnCriterion",
            description="Check if order can be returned"
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state != "delivered":
                logger.debug(f"Order {entity.entity_id} is not delivered, current state: {entity.state}")
                return False
            ...
            try:
                ...
                logger.warning(f"Invalid delivery date format for order {entity.entity_id}: {e}")
                return False
            ...
            if return_reason not in valid_reasons:
                logger.debug(f"Invalid return reason '{return_reason}' for order {entity.entity_id}")
                return False
            ...
            if return_reason == "changed_mind":
                ...
                return_period_for_changed_mind = kwargs.get("changed_mind_return_days", 7)
                changed_mind_deadline = delivery_date + timedelta(days=return_period_for_changed_mind)
                
                if current_date > changed_mind_deadline:
                    logger.debug(f"Changed mind return period expired for order {entity.entity_id}")
                    return False
            
            logger.debug(f"Order {entity.entity_id} can be returned for reason: {return_reason}")
            return True
            
        except Exception as e:
            logger.exception(f"Failed to check return criteria for order {entity.entity_id}")
