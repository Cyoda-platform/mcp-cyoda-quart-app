    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state != "inactive":
                logger.debug(f"Category {entity.entity_id} is not inactive, current state: {entity.state}")
                return False
            ...
            if active_pets_count > 0:
                logger.debug(f"Category {entity.entity_id} has {active_pets_count} active pets")
                return False
            
            # Check if category has pending orders (from kwargs or metadata)
            pending_orders_count = kwargs.get("pending_orders_count", 0)
            if pending_orders_count > 0:
                logger.debug(f"Category {entity.entity_id} has {pending_orders_count} pending orders")
                return False
            ...
            if not pets_validated or not orders_validated:
                ...
                if "active_pets_count" not in kwargs and "pending_orders_count" not in kwargs:
                    logger.debug(f"No pet/order counts provided, allowing archive for category {entity.entity_id}")
                    return True
                return False
            ...
            if deactivated_at:
                try:
                    ...
                    deactivation_date = datetime.fromisoformat(deactivated_at.replace('Z', '+00:00'))
                    ...
                    if current_date - deactivation_date < min_inactive_period:
                        logger.debug(f"Category {entity.entity_id} has not been inactive long enough")
                        return False
                        
                except (ValueError, TypeError) as e:
                    logger.warning(f"Invalid deactivation date format for category {entity.entity_id}: {e}")
            
            logger.debug(f"Category {entity.entity_id} can be archived")
            return True
            
        except Exception as e:
            logger.exception(f"Failed to check archive criteria for category {entity.entity_id}")
            raise CriteriaError(self.name, f"Failed to check category archive: {str(e)}", e)
