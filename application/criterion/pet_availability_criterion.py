    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if entity.state != "available":
                logger.debug(
                    f"Pet {entity.entity_id} is not available, current state: {entity.state}"
                )
                return False
            ...
            if entity.ownerId is not None:
                logger.debug(
                    f"Pet {entity.entity_id} is already reserved by owner {entity.ownerId}"
                )
                return False
            ...
            logger.exception(
                f"Failed to check availability criteria for pet {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check pet availability: {str(e)}", e
            )
