"""
Pet Routes for Purrfect Pets Application
...
async def list_pets(query_args: PetQueryParams) -> ResponseReturnValue:
    ...
    try:
        ...
        if query_args.age_min is not None or query_args.age_max is not None:
            filtered_list = []
            for pet_data in entity_list:
                age = pet_data.get("age", 0)
                if query_args.age_min is not None and age < query_args.age_min:
                    continue
                if query_args.age_max is not None and age > query_args.age_max:
                    continue
                filtered_list.append(pet_data)
