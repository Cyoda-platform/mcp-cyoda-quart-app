...
async def medical_hold_pet(pet_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        medical_reason = data.get("medical_reason", "Medical attention required") if data else "Medical attention required"
        ...
        if not response:
            return jsonify({"error": "Pet not found or cannot be placed on medical hold"}), 404
...
async def cancel_reservation(pet_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        cancellation_reason = data.get("cancellation_reason", "Reservation cancelled") if data else "Reservation cancelled"
        ...
        if not response:
            return jsonify({"error": "Pet not found or reservation cannot be cancelled"}), 404
