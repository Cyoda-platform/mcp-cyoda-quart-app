"""
DataAnalysis Routes for Cyoda Client Application

Manages all DataAnalysis-related API endpoints including CRUD operations
...
from ..entity.dataanalysis.version_1.dataanalysis import DataAnalysis
from ..models import (
    CountResponse,
...
@tag(["dataanalysis"])
@operation_id("create_dataanalysis")
@validate(
    request=DataAnalysis,
    responses={
        201: (DataAnalysisResponse, None),
        400: (ValidationErrorResponse, None),
        500: (ErrorResponse, None),
    },
)
async def create_dataanalysis(
    data: DataAnalysis,
) -> ResponseReturnValue:
    """Create a new DataAnalysis with comprehensive validation"""
    try:
        ...
        logger.warning("Validation error creating DataAnalysis: %s", str(e))
...
@dataanalysis_bp.route("/<entity_id>", methods=["GET"])
@tag(["dataanalysis"])
@operation_id("get_dataanalysis")
@validate(
    responses={
        200: (DataAnalysisResponse, None),
        404: (ErrorResponse, None),
        400: (ErrorResponse, None),
        500: (ErrorResponse, None),
    }
)
async def get_dataanalysis(entity_id: str) -> ResponseReturnValue:
    """Get DataAnalysis by ID with validation"""
    try:
        ...
        logger.exception("Error getting DataAnalysis %s: %s", entity_id, str(e))
        return {"error": str(e), "code": "INTERNAL_ERROR"}, 500


@dataanalysis_bp.route("", methods=["GET"])
@validate_querystring(DataAnalysisQueryParams)
@tag(["dataanalysis"])
@operation_id("list_dataanalysis")
@validate(
    responses={
        200: (DataAnalysisListResponse, None),
        400: (ValidationErrorResponse, None),
        500: (ErrorResponse, None),
    }
)
async def list_dataanalysis(
    query_args: DataAnalysisQueryParams,
) -> ResponseReturnValue:
    """List DataAnalysis with optional filtering and validation"""
    try:
        # Build search conditions based on query parameters
        search_conditions: Dict[str, str] = {}

        if query_args.data_source_id:
            search_conditions["dataSourceId"] = query_args.data_source_id

        if query_args.analysis_type:
            search_conditions["analysisType"] = query_args.analysis_type
...
@dataanalysis_bp.route("/<entity_id>", methods=["PUT"])
@validate_querystring(DataAnalysisUpdateQueryParams)
@tag(["dataanalysis"])
@operation_id("update_dataanalysis")
@validate(
    request=DataAnalysis,
    responses={
        200: (DataAnalysisResponse, None),
...
async def update_dataanalysis(
    entity_id: str, data: DataAnalysis, query_args: DataAnalysisUpdateQueryParams
) -> ResponseReturnValue:
    """Update DataAnalysis and optionally trigger workflow transition with validation"""
    try:
        # Validate entity ID format
        if not entity_id or len(entity_id.strip()) == 0:
            return (
        ...
        logger.info("Updated DataAnalysis %s", entity_id)
        ...
        logger.warning(
            "Validation error updating DataAnalysis %s: %s", entity_id, str(e)
        )
        return jsonify({"error": str(e), "code": "VALIDATION_ERROR"}), 400
    except Exception as e:
        logger.exception("Error updating DataAnalysis %s: %s", entity_id, str(e))
        return jsonify({"error": str(e), "code": "INTERNAL_ERROR"}), 500


@dataanalysis_bp.route("/<entity_id>", methods=["DELETE"])
@tag(["dataanalysis"])
@operation_id("delete_dataanalysis")
@validate(
    responses={
...
async def delete_dataanalysis(entity_id: str) -> ResponseReturnValue:
    """Delete DataAnalysis with validation"""
    try:
        # Validate entity ID format
        if not entity_id or len(entity_id.strip()) == 0:
            return (
        ...
        response = DeleteResponse(
            success=True,
            message="DataAnalysis deleted successfully",
            entityId=entity_id,
        )
        ...
        logger.exception("Error deleting DataAnalysis %s: %s", entity_id, str(e))
        return {"error": str(e), "code": "INTERNAL_ERROR"}, 500
...
async def check_exists(entity_id: str) -> ResponseReturnValue:
    """Check if DataAnalysis exists by ID"""
    try:
        ...
        logger.exception(
            "Error checking DataAnalysis existence %s: %s", entity_id, str(e)
        )
        return {"error": str(e)}, 500


@dataanalysis_bp.route("/count", methods=["GET"])
@tag(["dataanalysis"])
@operation_id("count_dataanalysis")
@validate(responses={200: (CountResponse, None), 500: (ErrorResponse, None)})
async def count_entities() -> ResponseReturnValue:
    """Count total number of DataAnalysis"""
    try:
        ...
        logger.exception("Error counting DataAnalysis: %s", str(e))
        return jsonify({"error": str(e)}), 500


@dataanalysis_bp.route("/<entity_id>/transitions", methods=["GET"])
@tag(["dataanalysis"])
@operation_id("get_dataanalysis_transitions")
@validate(
    responses={
...
async def get_available_transitions(entity_id: str) -> ResponseReturnValue:
    """Get available workflow transitions for DataAnalysis"""
    try:
        ...
        logger.exception(
            "Error getting transitions for DataAnalysis %s: %s", entity_id, str(e)
        )
        return jsonify({"error": str(e)}), 500
...
@validate(
    ...
    responses={
        200: (DataAnalysisSearchResponse, None),
        400: (ValidationErrorResponse, None),
        500: (ErrorResponse, None),
    },
)
async def search_entities(data: SearchRequest) -> ResponseReturnValue:
    """Search DataAnalysis using simple field-value search with validation"""
    try:
        ...
        logger.exception("Error searching DataAnalysis: %s", str(e))
        return {"error": str(e)}, 500
...
async def find_all_entities() -> ResponseReturnValue:
    ...
    try:
        ...
        logger.exception("Error finding all DataAnalysis: %s", str(e))
        return {"error": str(e)}, 500


@dataanalysis_bp.route("/<entity_id>/transitions", methods=["POST"])
@tag(["dataanalysis"])
@operation_id("trigger_dataanalysis_transition")
@validate(
...
async def trigger_transition(
    ...
    try:
        ...
        if not current_entity:
            return jsonify({"error": "DataAnalysis not found"}), 404
        ...
        logger.info(
            "Executed transition '%s' on DataAnalysis %s",
        ...
        logger.exception(
            "Error executing transition on DataAnalysis %s: %s", entity_id, str(e)
        )
        return jsonify({"error": str(e)}), 500
