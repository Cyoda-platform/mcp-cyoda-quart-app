...
from ..entity.product.version_1.product import Product
from ..models import (
    CountResponse,
    DeleteResponse,
    ErrorResponse,
...
async def list_products(query_args: ProductQueryParams) -> ResponseReturnValue:
    ...
    try:
        ...
        if query_args.category:
            search_conditions["categoryName"] = query_args.category
        if query_args.status:
            search_conditions["status"] = query_args.status
        if query_args.state:
            search_conditions["state"] = query_args.state
        if query_args.trend_indicator:
            search_conditions["trendIndicator"] = query_args.trend_indicator
        if query_args.restock_needed is not None:
            search_conditions["restockNeeded"] = str(query_args.restock_needed).lower()
...
async def search_products(data: SearchRequest) -> ResponseReturnValue:
    """Search Products using field-value search with validation"""
    try:
