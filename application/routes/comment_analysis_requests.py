...
from ..entity.comment_analysis_request.version_1.comment_analysis_request import (
...
async def get_comment_analysis_request(entity_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        if not response:
            return {
                "error": "CommentAnalysisRequest not found",
                "code": "NOT_FOUND",
            }, 404
...
async def update_comment_analysis_request(
    entity_id: str,
    data: CommentAnalysisRequest,
    query_args: CommentAnalysisRequestUpdateQueryParams,
) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.exception(
            "Error updating CommentAnalysisRequest %s: %s", entity_id, str(e)
        )
...
async def get_available_transitions(entity_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.exception(
            "Error getting transitions for CommentAnalysisRequest %s: %s",
            entity_id,
            str(e),
        )
        return jsonify({"error": str(e)}), 500
...
async def trigger_transition(
    ...
    try:
        ...
        logger.exception(
            "Error executing transition on CommentAnalysisRequest %s: %s",
            entity_id,
            str(e),
        )
        return jsonify({"error": str(e)}), 500
