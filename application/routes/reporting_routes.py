...
async def get_subscriber_statistics():
    ...
    try:
        ...
        active_subscribers = sum(1 for s in subscribers if s.data.get("isActive") and s.state == "active")
        pending_subscribers = sum(1 for s in subscribers if s.state == "pending")
        unsubscribed_subscribers = sum(1 for s in subscribers if s.state == "unsubscribed")
        ...
        for subscriber in subscribers:
            ...
            if subscription_date_str:
                try:
                    subscription_date = datetime.fromisoformat(subscription_date_str.replace('Z', '+00:00'))
...
async def get_email_delivery_statistics():
    ...
    try:
        ...
        successful_deliveries = sum(1 for d in deliveries if d.state in ["delivered", "opened"])
        failed_deliveries = sum(1 for d in deliveries if d.state in ["failed", "bounced"])
        ...
        bounce_rate = (failed_deliveries / total_emails_sent * 100) if total_emails_sent > 0 else 0
        delivery_rate = (successful_deliveries / total_emails_sent * 100) if total_emails_sent > 0 else 0
...
async def get_catfact_statistics():
    ...
    try:
        ...
        facts_distributed = sum(1 for cf in catfacts if cf.state in ["sent", "archived"])
        
        # Calculate average fact length
        fact_lengths = [cf.data.get("length", 0) for cf in catfacts if cf.data.get("length")]
        average_fact_length = sum(fact_lengths) // len(fact_lengths) if fact_lengths else 0
        ...
        for catfact in catfacts:
            ...
            if distributed_at:
                try:
                    dist_date = datetime.fromisoformat(distributed_at.replace('Z', '+00:00'))
            ...
            if scheduled_date_str and catfact.state == "scheduled":
                try:
                    ...
                    if not next_scheduled_distribution or scheduled_date < next_scheduled_distribution:
        ...
        result = {
            ...
            "lastDistributionDate": last_distribution_date.isoformat() if last_distribution_date else None,
            "nextScheduledDistribution": next_scheduled_distribution.isoformat() if next_scheduled_distribution else None
        }
