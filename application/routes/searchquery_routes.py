    ...
    try:
        ...
        result = {
            ...
            "meta": {
                ...
                "cache_expired": search_data.get("cachedAt") and SearchQuery(**search_data).is_cache_expired()
            }
        }
...
async def get_cached_results() -> ResponseReturnValue:
    ...
    try:
        ...
        if not query_hash:
            return jsonify({"error": "Query hash is required", "code": "MISSING_HASH"}), 400
        ...
        if not results:
            return jsonify({"error": "No cached results found", "code": "NOT_FOUND"}), 404
        ...
        if search_query.is_cache_expired():
            return jsonify({"error": "Cached results have expired", "code": "CACHE_EXPIRED"}), 410
...
async def trigger_search_transition(entity_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.info("Executed transition '%s' on SearchQuery %s", transition_name, entity_id)
        ...
        logger.exception("Error executing transition on SearchQuery %s: %s", entity_id, str(e))
        return jsonify({"error": str(e), "code": "INTERNAL_ERROR"}), 500
