"""
Email Campaign Routes for Cat Fact Subscription Application
...
async def list_email_campaigns(query_args: EmailCampaignQueryParams) -> ResponseReturnValue:
    ...
    try:
        ...
        if query_args.campaign_type:
            search_conditions["campaignType"] = query_args.campaign_type
...
@email_campaigns_bp.route("/<entity_id>/transitions", methods=["GET"])
@tag(["email-campaigns"])
@operation_id("get_email_campaign_transitions")
@validate(
    responses={
...
async def get_available_transitions(entity_id: str) -> ResponseReturnValue:
    """Get available workflow transitions for EmailCampaign"""
    try:
        ...
        logger.exception(
            "Error getting transitions for EmailCampaign %s: %s", entity_id, str(e)
        )
        return jsonify({"error": str(e)}), 500
...
async def search_entities(data: SearchRequest) -> ResponseReturnValue:
    """Search EmailCampaigns using simple field-value search"""
    try:
...
@email_campaigns_bp.route("/<entity_id>/transitions", methods=["POST"])
@tag(["email-campaigns"])
@operation_id("trigger_email_campaign_transition")
@validate(
...
async def trigger_transition(
    ...
    try:
        ...
        logger.info(
            "Executed transition '%s' on EmailCampaign %s",
        ...
        logger.exception(
            "Error executing transition on EmailCampaign %s: %s", entity_id, str(e)
        )
        return jsonify({"error": str(e)}), 500
