...
async def create_category(data: CategoryRequest):
    ...
    try:
        # Generate unique ID
        category_id = (
            len(await entity_service.find_all("Category", ENTITY_VERSION) or []) + 1
        )
        ...
        # Save category
        category_response = await entity_service.save(
            category_data, "Category", ENTITY_VERSION
        )
...
async def update_category(category_id: int, data: CategoryUpdateRequest):
    ...
    try:
        ...
        updated_category = (
            ...
            if isinstance(updated_category_data, dict)
            else updated_category_data
        )
        ...
        response = {
            ...
            "state": (
                updated_category.state.upper() if updated_category.state else "UNKNOWN"
            ),
