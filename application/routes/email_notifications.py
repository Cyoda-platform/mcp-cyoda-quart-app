...
async def create_email_notification(json: EmailNotification) -> tuple[Dict[str, Any], int]:
    """
...
async def get_email_notification(entity_id: str) -> tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        return {"error": "Failed to retrieve email notification", "details": str(e)}, 500


@email_notifications_bp.route("/<entity_id>", methods=["PUT"])
@tag(["EmailNotifications"])
@validate_json(EmailNotification)
async def update_email_notification(entity_id: str, json: EmailNotification) -> tuple[Dict[str, Any], int]:
    """
    ...
    try:
        ...
        if hasattr(response, 'entities') and response.entities:
            ...
            if analytics["total_emails"] > 0:
                analytics["delivery_rate"] = analytics["sent_emails"] / analytics["total_emails"] * 100
                analytics["average_retry_count"] = total_retries / analytics["total_emails"]
            
            if analytics["sent_emails"] > 0:
                analytics["open_rate"] = analytics["opened_emails"] / analytics["sent_emails"] * 100
