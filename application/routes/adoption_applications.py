...
adoption_applications_bp = Blueprint("adoption_applications", __name__, url_prefix="/api/adoption-applications")
...
async def update_adoption_application(application_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.warning(f"Validation error updating adoption application {application_id}: {str(e)}")
        return jsonify({"error": str(e)}), 400
    except Exception as e:
...
async def start_review(application_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        if not response:
            return jsonify({"error": "Adoption application not found or cannot start review"}), 404
        ...
        logger.error(f"Error starting review for adoption application {application_id}: {str(e)}")
...
async def approve_application(application_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        if not response:
            return jsonify({"error": "Adoption application not found or cannot be approved"}), 404
...
async def reject_application(application_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        rejection_reason = data.get("rejection_reason", "Application did not meet requirements") if data else "Application did not meet requirements"
        ...
        if not response:
            return jsonify({"error": "Adoption application not found or cannot be rejected"}), 404
...
async def withdraw_application(application_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        withdrawal_reason = data.get("withdrawal_reason", "Customer withdrew application") if data else "Customer withdrew application"
        ...
        if not response:
            return jsonify({"error": "Adoption application not found or cannot be withdrawn"}), 404
        ...
        logger.error(f"Error withdrawing adoption application {application_id}: {str(e)}")
