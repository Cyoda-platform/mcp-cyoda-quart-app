...
performance_reports_bp = Blueprint("performance_reports", __name__, url_prefix="/api/performance-reports")
...
async def create_performance_report(json: PerformanceReport) -> tuple[Dict[str, Any], int]:
    """
...
async def get_performance_report(entity_id: str) -> tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        return {"error": "Failed to retrieve performance report", "details": str(e)}, 500


@performance_reports_bp.route("/<entity_id>", methods=["PUT"])
@tag(["PerformanceReports"])
@validate_json(PerformanceReport)
async def update_performance_report(entity_id: str, json: PerformanceReport) -> tuple[Dict[str, Any], int]:
    """
...
async def generate_weekly_report() -> tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        logger.info(f"Initiated weekly report generation with ID: {response.metadata.id}")
        return result, 201
    ...
    try:
        ...
        if hasattr(response, 'entities') and response.entities:
            for entity_data in response.entities:
                ...
                if generation_time and (latest_timestamp is None or generation_time > latest_timestamp):
                    latest_report = {
                        "id": entity_data.metadata.id,
                        "state": entity_data.metadata.state,
                        "entity": entity
