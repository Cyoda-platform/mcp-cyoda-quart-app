...
@tag(["ProductData"])
@validate_json(ProductData)
async def update_product_data(entity_id: str, json: ProductData) -> tuple[Dict[str, Any], int]:
    """
...
async def trigger_data_extraction() -> tuple[Dict[str, Any], int]:
    ...
    try:
            ...
            for entity_data in response.entities:
                if entity_data.metadata.state == "validated":
                    try:
                        ...
                        logger.warning(f"Failed to trigger extraction for entity {entity_data.metadata.id}: {str(e)}")
        ...
        logger.info(f"Triggered data extraction for {processed_count} ProductData entities")
        return result, 200
