...
@users_bp.route("/by-business-id/<business_id>", methods=["GET"])
@tag(["users"])
@operation_id("get_user_by_business_id")
@validate(
    responses={
        200: (UserResponse, None),
        404: (ErrorResponse, None),
...
async def get_by_business_id(business_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.exception(
            "Error getting User by business ID %s: %s", business_id, str(e)
        )
        return jsonify({"error": str(e)}), 500
...
async def find_all_entities() -> ResponseReturnValue:
    ...
    try:
        ...
        logger.exception("Error finding all Users: %s", str(e))
        return {"error": str(e)}, 500
...
async def trigger_transition(
    ...
    try:
        ...
        logger.info(
            "Executed transition '%s' on User %s",
