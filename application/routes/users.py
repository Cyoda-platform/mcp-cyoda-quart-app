...
async def get_user(entity_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        # Thin proxy: return the entity directly (with sensitive fields removed)
...
async def list_users(query_args: UserQueryParams) -> ResponseReturnValue:
    ...
    try:
        ...
        if query_args.emailVerified is not None:
            search_conditions["emailVerified"] = str(query_args.emailVerified).lower()
        ...
        # Thin proxy: return entities directly (with sensitive fields removed)
...
async def update_user(
    ...
    try:
        ...
        # Add verification token to kwargs if provided
        kwargs = {}
        if query_args.verification_token:
            kwargs["verification_token"] = query_args.verification_token
        ...
        # Return updated entity directly (thin proxy, with sensitive fields removed)
        return _to_entity_dict(response.data), 200
...
async def search_entities(data: SearchRequest) -> ResponseReturnValue:
    ...
    try:
        ...
        # Thin proxy: return list of entities directly (with sensitive fields removed)
...
async def trigger_transition(entity_id: str, data: TransitionRequest) -> ResponseReturnValue:
    ...
    try:
        ...
        # Execute the transition with any additional parameters
