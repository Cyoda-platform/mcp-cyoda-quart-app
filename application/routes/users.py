...
async def delete_user(entity_id: str, data: TransitionRequest) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.info("Marked User %s for deletion", entity_id)

        # Return success message
        return {"message": "User account marked for deletion"}, 200
...
async def activate_user(entity_id: str, data: TransitionRequest) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.info("Activated User %s", entity_id)

        # Return updated entity directly (thin proxy)
        return _to_entity_dict(response.data), 200

    except Exception as e:
        logger.exception("Error activating User %s: %s", entity_id, str(e))
        return {"error": str(e), "code": "INTERNAL_ERROR"}, 500


@users_bp.route("/<entity_id>/suspend", methods=["POST"])
@tag(["users"])
...
async def suspend_user(entity_id: str, data: TransitionRequest) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.info("Suspended User %s", entity_id)

        # Return updated entity directly (thin proxy)
        return _to_entity_dict(response.data), 200

    except Exception as e:
        logger.exception("Error suspending User %s: %s", entity_id, str(e))
        return {"error": str(e), "code": "INTERNAL_ERROR"}, 500


@users_bp.route("/<entity_id>/reactivate", methods=["POST"])
@tag(["users"])
...
async def reactivate_user(
    ...
    try:
        ...
        logger.info("Reactivated User %s", entity_id)

        # Return updated entity directly (thin proxy)
        return _to_entity_dict(response.data), 200

    except Exception as e:
        logger.exception("Error reactivating User %s: %s", entity_id, str(e))
        return {"error": str(e), "code": "INTERNAL_ERROR"}, 500
