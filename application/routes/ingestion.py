...
async def _fetch_comments_from_api(post_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            logger.info(f"Fetched {len(comments_data)} comments from API for post {post_id}")
...
async def _create_comment_entities(comments_data: List[Dict[str, Any]]) -> List[str]:
    ...
    for comment_data in comments_data:
        try:
            ...
            logger.debug(f"Created Comment entity {response.metadata.id} for comment {api_comment.id}")
            
        except Exception as e:
            logger.warning(f"Failed to create Comment entity for comment {comment_data.get('id', 'unknown')}: {str(e)}")
...
async def _create_analysis_report(post_id: int, recipient_email: str) -> str:
    ...
    try:
        ...
        logger.info(f"Created CommentAnalysisReport entity {response.metadata.id} for post {post_id}")
        return response.metadata.id
        
    except Exception as e:
        logger.error(f"Failed to create CommentAnalysisReport for post {post_id}: {str(e)}")
        raise
