...
hnitemcollection_bp = Blueprint(
    "hnitemcollection", __name__, url_prefix="/api/hnitemcollection"
)
...
async def create_collection() -> ResponseReturnValue:
    ...
    try:
        ...
        return jsonify({"id": response.metadata.id, "status": "success"}), 201
...
async def firebase_pull() -> ResponseReturnValue:
    ...
    try:
        ...
        collection = HNItemCollection(name=collection_name, source="firebase_api")
...
async def get_collection_status(entity_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        if not response:
            return (
                jsonify({"error": "HNItemCollection not found", "code": "NOT_FOUND"}),
                404,
            )
        ...
        result = {
            ...
            "processing_summary": {
                ...
                "success_rate": collection_data.get("processedItems", 0)
                / max(collection_data.get("totalItems", 1), 1),
                "is_complete": (
                    collection_data.get("processedItems", 0)
                    + collection_data.get("failedItems", 0)
                )
                >= collection_data.get("totalItems", 0),
            },
        }
...
async def trigger_transition(entity_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.info(
            "Executed transition '%s' on HNItemCollection %s",
            transition_name,
            entity_id,
        )
        ...
        logger.exception(
            "Error executing transition on HNItemCollection %s: %s", entity_id, str(e)
        )
        return jsonify({"error": str(e), "code": "INTERNAL_ERROR"}), 500
