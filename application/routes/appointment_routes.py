...
async def get_appointments():
    ...
    try:
        ...
        for appointment in paginated_appointments:
            appointment_dict = appointment.dict() if hasattr(appointment, 'dict') else appointment
        ...
        return jsonify({'error': 'Failed to retrieve appointments', 'message': str(e)}), 500
...
async def get_appointment(appointment_id: str):
    ...
    try:
        ...
        try:
            appointment = await entity_service.find_by_business_id("Appointment", appointment_id)
        except:
        ...
        return jsonify({'error': 'Failed to retrieve appointment', 'message': str(e)}), 500
...
async def create_appointment():
    ...
    try:
        ...
        required_fields = ['pet_id', 'owner_id', 'vet_id', 'appointment_date', 'duration_minutes', 'appointment_type']
...
async def update_appointment(appointment_id: str):
    ...
    try:
        ...
        for key, value in data.items():
            if hasattr(appointment, key) and key not in ['appointment_id', 'technical_id', 'entity_id']:
                setattr(appointment, key, value)
        
        transition = data.get('transition')
        
        if transition:
            updated_appointment = await entity_service.update(appointment, transition=transition)
        else:
