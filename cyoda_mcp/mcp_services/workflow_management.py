...
class WorkflowManagementService:
    ...
    async def copy_workflows_between_entities(
        ...
        try:
            ...
            export_result = await self.export_entity_workflows(
                entity_name=source_entity_name, model_version=source_model_version
            )
            ...
            logger.info(
                f"Successfully copied {len(workflows)} workflows from {source_entity_name} to {target_entity_name}"
            )
            ...
            logger.exception(
                f"Failed to copy workflows from {source_entity_name} to {target_entity_name}: {e}"
            )
            return {
                "success": False,
                "error": str(e),
    ...
    async def validate_workflows(
        ...
        try:
            ...
            return await self.workflow_repository.validate_workflow_definitions(
                workflows
            )
