...
class FetchAndProcessDataProcessor(CyodaProcessor):
    ...
    def __init__(self, name: str = "fetch_and_process_data", description: str = ""):
        super().__init__(
            ...
            or "Fetches Nobel laureate data from API and processes it",
        )

    async def process(
        self, entity: CyodaEntity, payload: Optional[Dict[str, Any]] = None, **kwargs
    ) -> CyodaEntity:
        ...
        try:
            ...
            for laureate_data in valid_laureates:
                try:
                    ...
                    logger.exception(
                        f"Failed to update laureate id={laureate_data['id']}: {e}"
                    )
            ...
            logger.info(
                f"Successfully processed {len(valid_laureates)} laureates for job {entity.entity_id}"
            )
            return entity

        except Exception as e:
            logger.exception(
                f"Failed to fetch and process data for job {entity.entity_id}"
            )
            entity.set_state("FAILED")
            entity.add_metadata("completed_at", datetime.now(timezone.utc).isoformat())
            entity.add_metadata("error_message", str(e))
            raise ProcessorError(
                self.name, f"Failed to fetch and process data: {str(e)}", e
            )

    def can_process(self, entity: CyodaEntity, **kwargs) -> bool:
        """Check if this processor can handle the entity."""
        return (
            isinstance(entity, JobEntity) or entity.get_metadata("entity_type") == "job"
        )
