...
class ExampleEntityValidationCriterion(CyodaCriteriaChecker):
    ...
    async def check(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        ...
        try:
            ...
            # Check entity state - must be in 'created' state
            if entity.state != "created":
                self.logger.warning(
                    f"Entity {entity.technical_id} is not in 'created' state (current: {entity.state})"
                )
                return False

            # Validate required fields
            if not entity.name or len(entity.name.strip()) < 3 or len(entity.name) > 100:
                self.logger.warning(
