...
class EntityStateCriteria(CyodaCriteriaChecker):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            or f"Checks if entity is in one of the expected states: {expected_states}",
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            if isinstance(e, CriteriaError):
                raise
            logger.exception(
                f"Failed to check entity state criteria for entity {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check entity state criteria: {str(e)}", e
            )
