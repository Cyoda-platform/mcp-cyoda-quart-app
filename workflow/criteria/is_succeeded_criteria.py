...
class IsSucceededCriteria(CyodaCriteriaChecker):
    ...
    def __init__(self, name: str = "is_succeeded", description: str = ""):
        super().__init__(
            name=name, description=description or "Checks if a job entity has succeeded"
        )
    ...
    async def check(self, entity: CyodaEntity, **kwargs) -> bool:
        ...
        try:
            ...
            logger.exception(
                f"Failed to check success criteria for entity {entity.entity_id}"
            )
            raise CriteriaError(
                self.name, f"Failed to check success criteria: {str(e)}", e
            )
